...
3. **Download/register.** Run `scripts/ingest-article` **without** `--index` (default). File-watcher indexes async on the next RAG watch cycle. Reserve `--index` for ≤3 single-file/out-of-band ingests only — batch cursor-sdk dispatches must omit `--index` so the dispatch returns before the outer timeout (friction 23842).
...
5. **Index verification.** After watcher cycle (or after optional `--index`), run `rag(op="coverage")`; confirm relevant scope `indexed_files` and fresh `last_indexed` before declaring done. Do **not** treat "download+register finished" as dispatch-terminal when a cursor-sdk ingest is still running — confirm terminal per `consult-routing` § Post-dispatch output mutation gate before dedup/prune/trash on the same paths.
6. **Refresh corpus hints for the target scope.** After new content lands,
   call `rag(op="refresh_hints", arguments='{"scope": "<scope>"}')` (or
   `python -m services.rag.corpus_hints --scope <scope>`). Hints feed Pool B /
   vocab-aware retrieval. Freshness repair may refresh automatically on index,
   but agent ingest should not assume that — verify the MCP/admin path returns
   `scopes_updated` rather than 503. Full rebuilds: `tasks/runbooks/rag-post-index-refresh.md`.

Note: MCP `rag(op="search")` uses `rag-context` (`rewrite_enabled: false`), so
bare queries report `scope_source=default_scope` / `auto_classified=false` even
when hints are fresh. Prefer an explicit `scope=` for precision; do not treat
`auto_classified=false` alone as proof that hints are stale.
...
**Dispatch shape:** batch ingest via `team_dispatch(seat=cursor-sdk)` must call `scripts/ingest-article` without `--index`. Let the watcher index async; verify coverage after terminal closeout. Never fire corpus dedup/prune/trash on paths a still-live ingest dispatch may recreate — wait for dispatch terminal status first (`consult-routing` § Post-dispatch output mutation gate).
