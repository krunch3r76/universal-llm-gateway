...
Gate: `transcript_depth(S) ∈ {verbatim, light}` — read before composing the transcript. Structural file checks live in `session-close-audit.md`, not here.

## verbatim — dual layer

`transcript_messages = verbatim_layer ⊥ structural_layer`. Neither layer is derivable from the other.
...
| C5 | `|transcript_messages| > |summary|` |
...
Web: read `web-transcript-preprocessing` before assembly; strip payloads, keep metadata.

## Named sections — structural layer

Structural layer (`session_summary_md` for cursor/light; trailing `## Session Summary` block of web `transcript_messages`) may include sibling H2s. Server appends it with only trailing-newline trim; heading normalizer rewrites only the `## Session Summary` line. Bare `## Summary` and other siblings remain untouched.
