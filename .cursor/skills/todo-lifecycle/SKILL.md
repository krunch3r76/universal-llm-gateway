...
|---|---|---|
| `thin_seeder` | Scaffold todo / OPEN FORKS / corpus pointers; ¬ `implement_ready` | Grok 4.7 High (cursor) / Sonnet 5 (mobile) |
...
**Spine default check:** one merged check via `workflows.check_review` (`cursor/claude-fable-5-1` during Fable window; explicit terra/Sol pin only when operator names Other Models) after densify-close — the standing cross-identity falsifier (`decision:autonomous-work-item-spine`). ¬ dual GPT-5.5; ¬ web as check substitute. Code-lane live-source / `workspaces://` ⇒ `seat=cursor-sdk` + check_review default; API reviewer only when fully pre-staged (friction a24082). Densify-close `implement_ready` ≠ Gate-6 ratification; access-only REVISE ≠ Gate-6 close.
