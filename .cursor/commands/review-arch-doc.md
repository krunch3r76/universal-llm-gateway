...
`projects/.cursor/rules/handoff-dispatchers.mdc`: `web-claude`, `team-generate [model]`, plus reserved
...
Parse args. Map `web-claude` and `team-generate [model]` per the table above.
...
with `team-generate` or `web-claude`."
...
Additionally, when the dispatcher is `web-claude`, include these cortex skill
references in the block so the reviewer loads them on receipt:
...
- cortex://agent-skills/evidence-review-discipline.md     (every factual claim must trace to extracted docstrings/signatures)
- cortex://agent-skills/no-silent-inference.md            (don't fill `<!-- HUMAN: ... -->` markers or unsupported_claims with confident-sounding prose)
- cortex://agent-skills/prose-discipline.md               (editorial-improvement findings prose)
```

For `team-generate` dispatch, the cortex skill list is optional — the protocol's
default invariants composition is sufficient since frontier loads workspace rules
via its own MCP path. Include them anyway if consistency across dispatcher modes
...
For `web-claude`: write packet to
...
`type:review`, `agent:claude-web`, `doc:<doc-slug>`.
...
Source-of-truth per `projects/.cursor/rules/handoff-dispatchers.mdc`: agent_bus thread for
`web-claude`; iteration history (with `execution_id` per pass) for `team-generate`.
