...
∀ spec: author against `tasks/specs/_TEMPLATE.md`. Copy it, fill every
...
Specs live at `tasks/specs/{slug}.md` where `{slug}` is canonical per
`@plan-slug-coherence_ws` (binds spec basename ≡ `todo:`/`plan:` entity ≡
phase_dir). The todo entity links via `source_uri: tasks/specs/{slug}.md`.
