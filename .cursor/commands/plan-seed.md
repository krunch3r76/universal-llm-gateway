...
/plan-seed {slug} task={parent-arc}  # attach seeded todo to existing task container
```
...
Copy `tasks/specs/_TEMPLATE.md` → `tasks/specs/{slug}.md` and fill it with the
...
pipeline(op="run", pipeline_id="plan-seed",
         ...
         options={
           ...
           "task": "{parent-arc}",           # optional — child_of edge todo→task:{slug}
         })
```
...
edge in one call, returning structured per-call results. When `task` is set, also
creates `todo --child_of--> task:{task}` in the same run (parent must exist —
`/task-seed` first).
...
- Confirm the spec file exists: `fs`/read `tasks/specs/{slug}.md`.
