...
**Surface rule:** `/mcp/code` — bare paths require an explicit `sandbox` or a Share URI (`cortex://` / `workspaces://`). `/mcp/life` (life / web-anthropic toys) — durable writes default to cortex when `sandbox` is omitted; prefer `cortex://notes/...`; `workspaces` is not available.
<!-- /target:* -->
<!-- target:* -->
## Descriptor Reads — Rule-first (context discipline)
...
1. **Rule / SOT** — `cortex-essentials`, `handoff-dispatchers`+`dispatch-workflow`, `fs`/this rule, `agent-bus-discipline`, `tool-reference` §rag.
...
Full discipline + "already covered" map: `mcp-descriptor-read-discipline.mdc`.
<!-- /target:* -->
<!-- target:cursor -->
