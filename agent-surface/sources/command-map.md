...
| `/overhaul` | `.cursor/commands/overhaul.md` → 12-step directory pass; read `.cursor/skills/overhaul-program/SKILL.md` first | `fs` scan paths + `team_dispatch(handoff, role=web-consult)` for deep split **planning** + post-overhaul **review**, `team_dispatch(op=generate, seat=cursor-sdk, packet_path=…, contract=implement)` for approved **mechanical apply** (default; dense packet) + `scripts/modularize` / `doc-generate` when operator approves each call | Read/plan only: modularize scan output + propose split/review plan; no execution |
| `/claude-ai-sync` | plugin `commands/claude-ai-sync.md` → `scripts/cortex/claude-ai-sync-jupiter` (`recon` dumps `/mnt/skills` then 1:1 apply: uninstall extras / upload missing / replace stale); runbook `.cursor/skills/claude-ai-bundle-sync/SKILL.md` | `fs` regen + validate locally; `claude-ai-sync-jupiter recon` for CDP dump+apply | Out of scope (no claude.ai UI access) |
