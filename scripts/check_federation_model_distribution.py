...
def get_federation_status(
    ...
    try:
        response = requests.get(f"{master_url}/api/v1/federation/status", timeout=5)
...
def check_model_on_edges(
    model_id: str, master_url: str = "http://localhost:9999", num_requests: int = 10
) -> dict[str, int]:
...
def main() -> None:
    if len(sys.argv) < 2:
        ...
        print("\nExample:")
        print(
            "  check_federation_model_distribution.py phi-3-5-mini-instruct-q8-0-16384"
        )
        sys.exit(1)
