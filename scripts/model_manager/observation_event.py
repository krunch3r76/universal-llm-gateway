...
__all__ = [
    name
    for name in dir(_charter_events)
    if name.startswith("emit_") and not name.startswith("emit__")
] + [
    name
    for name in dir(_conveyor_events)
    if name.startswith("emit_") and not name.startswith("emit__")
]

for _name in __all__:
    if hasattr(_charter_events, _name):
        globals()[_name] = getattr(_charter_events, _name)
    elif hasattr(_conveyor_events, _name):
        globals()[_name] = getattr(_conveyor_events, _name)
