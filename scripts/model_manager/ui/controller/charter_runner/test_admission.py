...
def test_decide_idle_judgment_autonomous_cursor_executor_admits_worker() -> None:
    ...
    row = replace(_idle_row(), pickup_executor="cursor/grok-4.7")
    transition = decide(
        row,
        EnvFacts(substrate_up=True, has_wip=False, attendance="autonomous"),
        _open_caps(),
...
def _seed_6171(conn, *, attendance: str = "attended") -> RootLedgerRow:
    return seed_from_confirm(
        ...
        SeedConfirm(
            ...
            pickup_executor="cursor/grok-4.7",
            attendance=attendance,
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("G9 — implement · executor=cursor/grok-4.7", False),
        (
            "G2 — A + Gate-2 · executor=`cursor/grok-4.7` · executor_lane: judgment",
            False,
        ),
        (
    ...
    outcome = _tick_6171(
        ...
        [
            _turn(
                3,
                "CHECKPOINT — work",
                _tip_body("G9 — implement slice · executor=cursor/grok-4.7"),
            )
        ],
    )
