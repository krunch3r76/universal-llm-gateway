...
chose ``ADMIT_WORKER`` / ``cursor/grok-4.7``. Gate lives in ``kernel_tick`` after
...
def test_compatible_cursor_executor_still_admits(
    ...
    """Tip executor=cursor/grok-4.7 must not false-positive the refuse gate."""
    _seed(ledger)
    ...
    outcome = _tick(
        ...
        [
            _turn(
                ...
                "CHECKPOINT — tip",
                _tip(
                    "G9 — continue · executor=cursor/grok-4.7",
                    footer_executor="cursor/grok-4.7",
                ),
            )
        ],
    )
