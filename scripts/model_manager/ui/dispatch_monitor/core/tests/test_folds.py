...
def test_completed_closes_execution_id_sibling_ghost() -> None:
    ...
    model.apply(
        Event(
            "frontier.sdk.worker.progress",
            1_100,
            {"execution_id": "exec-ghost", "resolved_model": "cursor/grok-4.7"},
        )
    )
    model.apply(
        Event(
            "frontier.sdk.worker.progress",
            1_200,
            {"dispatch_id": "real-dispatch", "resolved_model": "cursor/grok-4.7"},
        )
    )
    model.apply(
        Event(
...
def test_toolcall_updates_last_tool_and_idle() -> None:
    ...
    model.apply(
        Event(
            "frontier.sdk.worker.progress",
            1_000,
            {"dispatch_id": "d1", "resolved_model": "cursor/grok-4.7"},
        )
    )
    model.apply(
        Event(
...
def test_queued_and_generate_requested_carry_model() -> None:
    ...
    assert queued.model == "cursor/grok-4.7"
