...
from claude_bundles.catalog import (  # noqa: E402
...
def main(argv: list[str] | None = None) -> int:
    ...
    catalog_path = root / "config" / "skills.yaml"
    _ = args.check
    try:
        catalog = load_skill_catalog(
            path=catalog_path, repo_root=root, validate_sot=True
        )
        uris = catalog_source_uris(repo_root=root)
    except CatalogValidationError as exc:
        print(f"FAIL skill catalog parity: {exc}", file=sys.stderr)
        return 1
