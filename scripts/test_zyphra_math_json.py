...
def test_zyphra_math_verification(
    claim: str, expected_verdict: bool, base_url: str = "http://localhost:9999"
) -> dict[str, bool | str]:
    """
...
def main() -> int:
    ...
    if result["verdict_correct"]:
        print(
            f"✅ Verdict is CORRECT: {result['actual_verdict']} == {result['expected_verdict']}"
        )
        return 0
    else:
        print(
            f"⚠️  Verdict is INCORRECT: {result['actual_verdict']} != {result['expected_verdict']}"
        )
        print(
