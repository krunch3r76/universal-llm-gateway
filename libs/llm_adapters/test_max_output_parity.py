...
_RESPONSES_MODELS: tuple[str, ...] = ("gpt-5.5", "grok-4.3")
...
def _build_reasoning_table() -> dict[str, Any]:
    ...
    for model in ("xai/grok-4.3", "openai/gpt-5.5", "anthropic/claude-opus-4-8"):
        table[f"default_effort|{model}"] = default_reasoning_effort(model)
    for model in ("grok-4.3", "grok-4.20-0309-non-reasoning", "grok-3-mini"):
