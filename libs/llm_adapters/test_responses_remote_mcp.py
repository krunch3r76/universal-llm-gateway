...
- xAI raises RemoteMcpUnsupportedError (policy gate while mcp_remote_connector=False)
"""
...
def _mcp_env(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("MCP_PUBLIC_URL", "https://mcp.example.com/mcp/code/")
...
_EXPECTED_ENTRY = {
    "type": "mcp",
    "server_url": "https://mcp.example.com/mcp/code/",
    "server_label": "vortex",
...
def test_remote_mcp_xai_raises_unsupported() -> None:
    req = _base_req("grok-4.7", remote_mcp=True)
    with pytest.raises(
