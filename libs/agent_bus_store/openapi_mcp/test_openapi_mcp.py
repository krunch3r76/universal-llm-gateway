...
@pytest.mark.offline
def test_extract_typed_routes_returns_twenty_one_ops() -> None:
    ...
    assert "substrate_entity_mint" not in routes
...
def test_unbound_ops_are_collision_losers_only() -> None:
    ...
    assert "substrate_entity_mint" not in unbound
...
def test_generator_dry_run_covers_served_ops() -> None:
    schema = create_app().openapi()
    manifest = dry_run_generate(schema)
    assert len(manifest.served_ops) == 21
