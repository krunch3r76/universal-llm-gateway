...
See ``followup_resolve`` for the identity ladder. Never registers a lane,
``goto``s a CSE URL, or falls back to bus NOTE delivery.
"""
...
async def execute_followup(
    ...
    try:
        ...
        if found is None:
            resp = fail_followup(
                "cse_not_found_on_lane", detail="page gone before paste"
            )
            emit_followup_event(
                cdp_ask_followup_paste_verified(
                    registration_id=target.registration_id,
        ...
        if normalize_cse_url(page.url or "") != normalize_cse_url(target.chat_url):
            resp = fail_followup("lane_not_attached", detail=lane_not_attached_detail())
            emit_followup_event(
                cdp_ask_followup_paste_verified(
                    registration_id=target.registration_id,
                    resolution_path=resolution_path,
