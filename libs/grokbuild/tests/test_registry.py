...
async def test_cwds_under_exact_match(
    ...
    monkeypatch.setattr(
        "grokbuild.registry.REGISTRY_PATH", Path(str(tmp_path)) / "r.json"
    )
    root = str(tmp_path)
...
async def test_acquire_writes_cwd_to_disk(
    ...
    monkeypatch.setattr("grokbuild.registry.REGISTRY_PATH", reg)
...
def test_startup_prunes_stale_entries_when_pid_gone(
    ...
    dead_pid = 99999999
    with patch("grokbuild.registry._pid_running", return_value=False):
...
def test_startup_recovers_entries_when_pid_alive(
    ...
    with patch("grokbuild.registry._pid_running", return_value=True):
...
def test_startup_no_file_emits_zero_zero_event(
    ...
    events: list[tuple[str, dict[str, Any]]] = []
    monkeypatch.setattr(
        "grokbuild.events_core.record", lambda sig, **kw: events.append((sig, kw))
    )
    _load_registry_from_disk()
...
def test_schema_mismatch_discards_entries(
    ...
    """A registry file with a stale schema_version is treated as a stale file.

    Per [universal:no-bc] (sole-maintainer policy): there is no in-place
    migration. Entries are pruned and surfaced via the recovery event so
    operators can see what was discarded; ``_in_flight`` stays empty.
    """
    reg = tmp_path / "registry.json"
    monkeypatch.setattr("grokbuild.registry.REGISTRY_PATH", reg)
    reg.write_text(
