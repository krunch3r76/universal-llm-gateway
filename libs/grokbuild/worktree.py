...
The ``worktree_remove`` op lives in ``grokbuild.worktree_remove`` to keep
...
from grokbuild.worktree_schema import (  # noqa: F401 — re-exported for consumers
    _GIT_TIMEOUT,
    _NAME_VALID_RE,
...
async def worktree_create_op(
    *,
    ...
    AFTER admission passes (rejected dispatches emit only ``.rejected``,
    carrying their own correlation fields). Aligned across worktree ops
    in V2 close-out (review W9).
    """
    ...
    try:
        ...
        return _emit_create_failed(
            ...
            t0=t0,
            exit_code=None,
            stderr=f"git worktree add timed out after {_GIT_TIMEOUT}s",
            name=name,
            branch=branch,
            source_repo=source_repo,
            worktree_path=vr.worktree_path,
