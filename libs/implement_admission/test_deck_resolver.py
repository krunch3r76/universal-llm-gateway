...
_DENSE_SPEC = """# Dense phase fallback spec

## Problem

Missing deck on disk.
...
Use dense-spec fallback.

## Acceptance criteria

1. Dense spec body materializes.
...
def _write_dense_spec(
    root: Path,
    slug: str,
    text: str = _DENSE_SPEC,
    *,
    use_cortex_layout: bool = True,
) -> Path:
    if use_cortex_layout:
        spec_dir = root / "notes" / "system" / "specs"
    else:
        spec_dir = root / "tmp" / "prompts" / slug
...
def test_resolve_dense_spec_fallback_via_dense_spec_uri(
    tmp_path: Path, cortex_root: Path
) -> None:
    _ = cortex_root
    slug = "dense-fallback"
    _write_dense_spec(tmp_path, slug)
    pointer = f"cortex://notes/system/specs/{slug}.md"
    deck = resolve_phase_deck(
        _ref(f"plan:{slug}/phase-1"),
        workspaces_root=tmp_path,
        entity_attrs={"dense_spec_uri": pointer},
    )
    assert deck.body == _DENSE_SPEC
    assert deck.sha256.startswith("sha256:")
    expected = "sha256:" + hashlib.sha256(_DENSE_SPEC.encode("utf-8")).hexdigest()
    assert deck.sha256 == expected


def test_resolve_dense_spec_fallback_via_source_uri(
    tmp_path: Path, cortex_root: Path
) -> None:
    _ = cortex_root
    slug = "source-uri-fallback"
    _write_dense_spec(tmp_path, slug)
    deck = resolve_phase_deck(
        _ref(f"plan:{slug}/phase-1"),
        workspaces_root=tmp_path,
...
def test_resolve_dense_spec_uri_precedence_over_source_uri(
    tmp_path: Path, cortex_root: Path
) -> None:
    _ = cortex_root
    slug = "pointer-precedence"
    _write_dense_spec(tmp_path, slug, text=_DENSE_SPEC)
    alt = _DENSE_SPEC + "\n## Extra\n\nalternate body.\n"
    alt_path = tmp_path / "notes" / "system" / "specs" / "alternate.md"
    ...
    deck = resolve_phase_deck(
        ...
        workspaces_root=tmp_path,
        entity_attrs={
            "dense_spec_uri": f"cortex://notes/system/specs/{slug}.md",
        },
        source_uri="cortex://notes/system/specs/alternate.md",
    )
    assert deck.body == _DENSE_SPEC


def test_resolve_dense_spec_hash_changes_when_body_changes(
    tmp_path: Path, cortex_root: Path
) -> None:
    _ = cortex_root
    slug = "hash-bind"
    _write_dense_spec(tmp_path, slug)
    pointer = f"cortex://notes/system/specs/{slug}.md"
    first = resolve_phase_deck(
        _ref(f"plan:{slug}/phase-1"),
        workspaces_root=tmp_path,
        entity_attrs={"dense_spec_uri": pointer},
    )
    _write_dense_spec(tmp_path, slug, text=_DENSE_SPEC + "\nchanged.\n")
    second = resolve_phase_deck(
        _ref(f"plan:{slug}/phase-1"),
        workspaces_root=tmp_path,
        entity_attrs={"dense_spec_uri": pointer},
    )
...
def test_resolve_deck_file_wins_over_dense_spec(
    tmp_path: Path, cortex_root: Path
) -> None:
    _ = cortex_root
    slug = "deck-wins"
    _write_deck(tmp_path, slug, "phase-1-sample.md")
    _write_dense_spec(tmp_path, slug)
    deck = resolve_phase_deck(
        _ref(f"plan:{slug}/phase-1"),
        workspaces_root=tmp_path,
    ...
    assert deck.body.startswith("# Phase 1: sample")
    assert "Dense phase fallback spec" not in deck.body


def test_resolve_not_found_without_deck_or_dense_spec(tmp_path: Path) -> None:
