...
def test_todo_adapter_closes_in_process() -> None:
    ...
    def fake_dispatch(tool: str, args: dict) -> dict:
        ...
        if tool == "assert":
            return {"id": 1}
        if tool == "entity_get":
            return {"id": args["entity_id"], "attributes": {}}
    ...
    # Ordinary todos: sidecar -> assert -> entity_get attrs -> workflow_state=done.
    assert tools == ["todo_close_sidecar", "assert", "entity_get", "entity_update"]
    assert calls[1][1]["entity_id"] == "todo:my-todo"
    assert calls[3][1] == {"entity_id": "todo:my-todo", "workflow_state": "done"}
    assert (
        "cortex://notes/system/todos/my-todo-closure.md" in calls[1][1]["evidence_uris"]
    )
    assert results[0].status == "complete"
    assert "workflow_state=done" in (results[0].mutation or "")
...
def test_todo_adapter_withholds_done_for_path_sim_arc(attrs: dict) -> None:
    """a:26246 — Stage-B must not flip done before G6 for path-sim arcs."""
    calls: list[tuple[str, dict]] = []
    ...
    def fake_dispatch(tool: str, args: dict) -> dict:
        ...
        if tool == "todo_close_sidecar":
            return {
                "closure_summary_uri": (
                    "cortex://notes/system/todos/path-sim-todo-closure.md"
                )
            }
        if tool == "assert":
            return {"id": 1}
        if tool == "entity_get":
            return {"id": args["entity_id"], "attributes": attrs}
        if tool == "entity_update":
            return {"workflow_state": "done"}
        return {}
    ...
    closeout = ImplementCloseout(
        status=CloseoutStatus.COMPLETE,
        summary="stage-b evidence",
        source_ref="todo:path-sim-todo",
    )
    source = Source(
        source_ref="todo:path-sim-todo",
        canonical_ref="todo:path-sim-todo",
        source_kind=SourceKind.TODO,
    )
    results = run_adapters(closeout, source)
    tools = [c[0] for c in calls]
    assert tools == ["todo_close_sidecar", "assert", "entity_get"]
    assert "entity_update" not in tools
    assert results[0].status == "complete"
    assert "withheld done" in (results[0].mutation or "")
    assert "pending G6" in (results[0].mutation or "")
    assert calls[1][1]["entity_id"] == "todo:path-sim-todo"
...
def test_apply_returns_list_and_status_reconcile() -> None:
    ...
    import implement_admission.closeout as closeout_mod

    closeout_mod.ADAPTERS["todo"] = ListAdapter()
