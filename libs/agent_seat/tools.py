...
CORTEX_TOOL_DEFINITION: dict[str, Any] = _fn(
    ...
    "  session_close (session_id, agent, transcript_jsonl_path, session_summary_md, summary, domains?, decisions?, open_items?, entity_ids?, prior_session_id?, handoff_prompt?, assistant_label?, dry_run?)\n"
    "    — atomic server-side close. Server reads the Cursor agent-transcripts JSONL at\n"
    "    transcript_jsonl_path (sandbox: CURSOR_AGENT_TRANSCRIPTS_ROOT), assembles the verbatim\n"
    "    layer, appends the agent-composed session_summary_md (must start with '## Session\n"
    "    Summary'), and atomically writes file + entity + journal row + continues edge +\n"
    "    optional handoff entry. transcript_md (pre-Phase-2 arg) is gone. Response includes\n"
    "    transcript_entity_id, content_hash (sha256:<hex> — quote as provenance), turn_count,\n"
    "    byte_count, journal_row_id, and handoff_entry_id (int|null).\n"
