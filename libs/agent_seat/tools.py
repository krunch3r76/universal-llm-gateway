...
_CORTEX_OPS_DOC = (
    ...
    "  assemble_transcript (jsonl_path, session_id, agent?, assistant_label?) — Assemble a verbatim transcript layer from a Cursor JSONL. Args: jsonl_path: path under ``CURSOR_AGENT_TRANSCRIPTS_ROOT`` (absolute or relative to the root).  Sandbox-enforced — paths outside the root return ``{\"error\", \"reason\": \"path_outside_root\"}``. session_id: ``{agent}-YYYY-MM-DD-HHMMSS-{3hex}`` — appears in the H1 line. agent: cosmetic; echoed in the response. assistant_label: heading label for assistant blocks; default ``\"Assistant\"``. Returns: ``{\"verbatim_md\", \"turn_count\", \"byte_count\", \"agent\"}`` on success, ``{\"error\", \"reason\"}`` otherwise.  ``verbatim_md`` is the verbatim layer ONLY — inspect via this probe; do not pass it to ``session_close`` (use ``transcript_jsonl_path`` or ``transcript_messages*`` instead).\n"
    ...
    "  assertion_update (assertion_id, superseded_by?, valid_from?, valid_until?, confidence?, confidence_score?, review_status?, reviewer?, reviewed_at?, review_notes?, evidence_uris?, reasoning_summary?, predicate_form?, prospective_summary?, events_json?, force?, session_id?)\n"
    ...
    "  predicate_renormalize (dry_run?, sample_limit?) — Run dry_run_stratify and optional T0 flag-fields-only clear pass.\n"
