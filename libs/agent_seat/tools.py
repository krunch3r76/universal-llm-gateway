...
CORTEX_TOOL_DEFINITION: dict[str, Any] = _fn(
    ...
    "  session_close (session_id, agent, transcript_md, summary, domains?, decisions?, open_items?, entity_ids?, prior_session_id?, handoff_prompt?, dry_run?)\n"
    "    — atomic close: transcript file + entity + journal row + continues edge in one call.\n"
    "    Optional handoff_prompt is the agent-authored continuation prompt for the next session;\n"
    "    server-side it lands as a reflective journal entry (kind='handoff', register='self')\n"
    "    linked to the transcript via link_type='handoff_for'. Response includes\n"
    "    transcript_entity_id and handoff_entry_id (int|null).\n"
