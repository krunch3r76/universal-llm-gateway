...
def skills_conn(tmp_path: Path) -> sqlite3.Connection:
    ...
    _insert_skill(
        ...
        applicable_agents=["*"],
        capabilities_required=["mcp_life"],
    )
...
def test_t1_boot_skills_index_envelope(skills_conn: sqlite3.Connection) -> None:
    with patch("cortex_store.routes.skills.cortex_conn", return_value=skills_conn):
        body = get_skills(limit=50, for_agent="claude-web", view="boot")
...
def test_t2_capability_all_of_excludes_life_from_code_required() -> None:
    def _seed() -> sqlite3.Connection:
        ...
        _insert_skill(
            ...
            "agent_skill:mcp-only",
            applicable_agents=["*"],
            capabilities_required=["mcp_code"],
        )
        conn.commit()
        return conn

    with patch("cortex_store.routes.skills.cortex_conn", return_value=_seed()):
        life = get_skills(limit=50, for_agent="claude-web")
    with patch("cortex_store.routes.skills.cortex_conn", return_value=_seed()):
        code = get_skills(limit=50, for_agent="gpt-cursor")
    life_ids = {i["id"] for i in life["items"]}
    code_ids = {i["id"] for i in code["items"]}
    assert "agent_skill:mcp-only" not in life_ids
    assert "agent_skill:mcp-only" in code_ids
...
def test_t4_write_guard_unknown_slug_create(skills_conn: sqlite3.Connection) -> None:
    payload = EntityCreate(
        id="agent_skill:rename-forward",
        type="agent_skill",
        name="rename-forward",
        attributes={"applicable_agents": ["web-anthropic"]},
    ).model_dump()
    out = create_entity_impl(skills_conn, payload, commit=False)
    assert out["id"] == "agent_skill:rename-forward"
...
def test_t6_write_guard_unknown_slug_update(skills_conn: sqlite3.Connection) -> None:
    ...
    skills_conn.commit()
    out = update_entity_impl(
        skills_conn,
        entity_id="agent_skill:update-me",
        updates={"attributes": {"applicable_agents": ["web-anthropic"]}},
        commit=False,
    )
    assert out["id"] == "agent_skill:update-me"
...
def test_t8_entities_default_allow_null_included(skills_conn: sqlite3.Connection) -> None:
    ...
    assert "agent_skill:null-entity" in ids


def test_t9_entities_all_skills_visible_for_web(
    ...
    ids = {item["id"] for item in out["items"]}
    assert "agent_skill:cursor-only" in ids
...
def test_tg2_rule_cursor_only_visible_for_web(skills_conn: sqlite3.Connection) -> None:
    _insert_rule(
        skills_conn,
        "rule:cursor-conduct",
    ...
    assert "rule:cursor-conduct" in {i["id"] for i in body["items"]}
...
def test_tg8_rule_capability_required_excludes_life_from_code() -> None:
    def _seed() -> sqlite3.Connection:
        conn = _conn()
        _insert_rule(
            conn,
    ...
    with patch("cortex_store.routes.skills.cortex_conn", return_value=_seed()):
        life = get_skills(limit=50, layer="rules", for_agent="claude-web")
    with patch("cortex_store.routes.skills.cortex_conn", return_value=_seed()):
        code = get_skills(limit=50, layer="rules", for_agent="gpt-cursor")
    life_ids = {i["id"] for i in life["items"]}
    code_ids = {i["id"] for i in code["items"]}
    assert "rule:mcp-only" not in life_ids
    assert "rule:mcp-only" in code_ids
