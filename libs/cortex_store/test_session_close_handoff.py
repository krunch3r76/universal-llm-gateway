...
def _write_jsonl_with_timestamp(path: Path, *, stamped: str) -> None:
    """JSONL whose first user turn carries a stable ``<timestamp>`` tag."""
    path.parent.mkdir(parents=True, exist_ok=True)
    records = [
        {
            "role": "user",
                ...
                "content": [
                    {
                        "type": "text",
                        "text": (
                            f"<timestamp>{stamped}</timestamp>\n"
                            "Please continue the handoff capture arc."
                        ),
                    }
                ]
            },
        },
        {
            ...
            "message": {
                "content": [
                    {"type": "text", "text": "Acknowledged — anchoring session_id."}
                ]
            },
        },
    ]
...
def test_preflight_returns_copy_paste_session_id_when_supplied_differs(
    ...
    _write_jsonl_with_timestamp(
        jsonl_path, stamped="2026-05-27T10:00:00+00:00"
    )
    boot_id = "cursor-2099-12-31-235959-fff"
    ...
    # Boot-held ID stays authoritative; JSONL-start is exposed for no-boot path.
    assert "session_id_from_jsonl_start" in result
    assert result["session_id_from_jsonl_start"].startswith("cursor-2026-05-27-100000-")
    assert result.get("session_id") is None or result.get("session_id") == boot_id
    assert "session_id_timing_note" in result
    assert not any(
        isinstance(w, str) and "differs from JSONL session-start" in w
...
def test_derive_session_id_parses_cursor_abbrev_month_with_tz() -> None:
    ...
    raw = "Thursday, Jul 9, 2026, 2:37 AM (UTC-7)"
    assert _normalize_cursor_timestamp(raw) == "2026-07-09 09:37:00"

    import tempfile

    with tempfile.TemporaryDirectory() as tmp:
        path = Path(tmp) / "t.jsonl"
        _write_jsonl_with_timestamp(path, stamped=raw)
        first = derive_session_id_from_jsonl_start(jsonl_path=path, agent="claude-cursor")
        second = derive_session_id_from_jsonl_start(jsonl_path=path, agent="claude-cursor")
        assert first is not None
        assert session_id_time_base(first) == "claude-cursor-2026-07-09-093700"
        assert session_id_time_base(first) == session_id_time_base(second)


def test_preflight_keeps_boot_session_id_when_jsonl_lacks_stable_start(
    session_env: dict[str, Path],
) -> None:
    """Friction 23205 / 21760: no <timestamp> + no birthtime ⇒ keep boot ID.

    Must not re-mint from st_mtime across successive preflight calls while the
    live-appended JSONL grows (Linux mtime race).
    """
    import os
    import time
    ...
    jsonl_path = session_env["transcripts_root"] / "cursor-uuid" / "cursor-uuid.jsonl"
    _write_jsonl(jsonl_path)
    boot_id = "claude-cursor-2026-07-09-093801-2fd"
    summary = _session_summary("Boot-held session_id when JSONL has no start tag.")
    first = ops_journals._op_session_close_preflight(
        session_id=boot_id,
        agent="claude-cursor",
        transcript_jsonl_path=str(jsonl_path),
        session_summary_md=summary,
        summary="Boot-held session_id when JSONL has no start tag.",
    )
    assert first["ok"] is True
    assert first.get("session_id") is None or first.get("session_id") == boot_id
    assert "session_id_from_jsonl_start" not in first
    assert first.get("jsonl_start_reason") == "missing_jsonl_start_timestamp"
    assert not any(
        isinstance(w, str) and "differs from JSONL session-start" in w
        for w in (first.get("warnings") or [])
    )

    # Grow the file + bump mtime; second preflight must stay pinned.
    time.sleep(1.05)
    ...
    second = ops_journals._op_session_close_preflight(
        session_id=boot_id,
        agent="claude-cursor",
        transcript_jsonl_path=str(jsonl_path),
        session_summary_md=summary,
    ...
    assert second.get("jsonl_start_reason") == "missing_jsonl_start_timestamp"
    assert "session_id_from_jsonl_start" not in second
    # Caller-supplied boot ID remains authoritative (not overwritten).
    assert second.get("session_id") is None or second.get("session_id") == boot_id
    assert session_id_time_base(boot_id) == session_id_time_base(boot_id)
