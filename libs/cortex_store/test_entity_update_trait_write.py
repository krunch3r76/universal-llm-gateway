...
def test_update_readback_skips_entity_access_log() -> None:
    conn = _conn()
    update_entity_impl(conn, entity_id="decision:x", updates={"name": "X renamed"})
    count = conn.execute("SELECT COUNT(*) AS n FROM entity_access_log").fetchone()["n"]
    assert count == 0
