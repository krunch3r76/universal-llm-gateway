...
from cortex_store.entity_rekey_core import (
    check_foreign_keys,
    entity_rekey_impl,
    entity_retype_impl,
)
...
def test_retype_changes_type_reprefixes_id_and_preserves_cross_type_alias(
    conn: sqlite3.Connection,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    peer = "project:retype-peer"
    _seed_entity(conn, peer)
    old_id = "agent_skill:markdown-nav"
    _seed_entity(conn, old_id, entity_type="agent_skill")
    ...
    result = entity_retype_impl(conn, old_id, "rule")

    new_id = "rule:markdown-nav"
    assert result["new_id"] == new_id
    assert result["old_type"] == "agent_skill"
    assert result["new_type"] == "rule"

    row = conn.execute(
        "SELECT type FROM entities WHERE id = ?", (new_id,)
    ).fetchone()
    assert row is not None
    assert row[0] == "rule"
    assert not conn.execute(
    ...
    fetched = _op_entity_get(entity_id=old_id)
    assert fetched["id"] == new_id
    assert fetched["type"] == "rule"


def test_retype_same_type_rejected_422(conn: sqlite3.Connection) -> None:
    _seed_entity(conn, "agent_skill:already", entity_type="agent_skill")
    with pytest.raises(HTTPException) as exc_info:
        entity_retype_impl(conn, "agent_skill:already", "agent_skill")
    assert exc_info.value.status_code == 422


def test_retype_collision_new_id_exists_409(conn: sqlite3.Connection) -> None:
    _seed_entity(conn, "rule:clash", entity_type="rule")
    _seed_entity(conn, "agent_skill:clash", entity_type="agent_skill")
    with pytest.raises(HTTPException) as exc_info:
        entity_retype_impl(conn, "agent_skill:clash", "rule")
    assert exc_info.value.status_code == 409
...
def test_merge_redirect_and_raw_id_tombstone(
    ...
    tombstone = _op_entity_get(entity_id=source, raw_id=True, intent="full")
