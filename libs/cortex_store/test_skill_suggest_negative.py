...
def _insert(conn: sqlite3.Connection, entity_id: str, **attrs: object) -> None:
    source_uri = attrs.pop("source_uri", f"agent-skills/{entity_id.split(':',1)[-1]}.md")
    payload = {
...
async def test_malformed_trigger_terms_skipped() -> None:
    conn = _conn()
    conn.execute(
        "INSERT INTO entities (id, type, name, source_uri, attributes) VALUES (?, 'agent_skill', 'bad', 'agent-skills/bad.md', ?)",
        (
            "agent_skill:bad",
            json.dumps(
                {
...
async def test_missing_source_uri_excluded() -> None:
    conn = _conn()
    conn.execute(
        "INSERT INTO entities (id, type, name, source_uri, attributes) VALUES ('agent_skill:nouri', 'agent_skill', 'nouri', NULL, ?)",
