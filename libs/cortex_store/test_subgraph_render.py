...
def test_hops_bound_strict_at_1():
    ...
    conn.commit()
    res = render_subgraph(conn, _ROOT, hops=1, **_FULL)
...
def test_edge_type_filter_excludes_unfiltered_paths():
    conn = make_test_conn()
    seed_grokbuild_graph(conn)
    res = render_subgraph(conn, _ROOT, edge_types=["depends_on"], **_FULL)
    ids = {e.entity_id for e in res.entities}
...
def test_markdown_contains_spec_skeleton():
    conn = make_test_conn()
    seed_grokbuild_graph(conn)
    md = render_subgraph(conn, _ROOT, hops=1, **_FULL).rendered
    for needle in (
...
def test_markdown_renders_empty_sentinel_when_no_related():
    ...
    conn.commit()
    res = render_subgraph(conn, "todo:alone", hops=1, **_FULL)
...
def test_markdown_deterministic_byte_stable():
    conn = make_test_conn()
    seed_grokbuild_graph(conn)
    r1 = render_subgraph(conn, _ROOT, hops=1, **_FULL).rendered
    r2 = render_subgraph(conn, _ROOT, hops=1, **_FULL).rendered
    assert r1 == r2
    conn.close()
...
def test_markdown_escapes_user_content_headings():
    ...
    conn.commit()
    md = render_subgraph(conn, "decision:weird", hops=1, **_FULL).rendered
...
def test_include_superseded_toggle():
    conn = make_test_conn()
    seed_grokbuild_graph(conn)
    off = render_subgraph(conn, _ROOT, include_superseded=False, **_FULL)
    on = render_subgraph(conn, _ROOT, include_superseded=True, **_FULL)
    off_claims = [
        a["claim"]
...
def test_events_carry_render_id_correlation(monkeypatch):
    ...
    conn = make_test_conn()
    seed_grokbuild_graph(conn)
    render_subgraph(conn, _ROOT, **_FULL)
...
def test_route_and_op_envelope_parity(tmp_path, monkeypatch):
    ...
    conn.close()
    monkeypatch.setattr("cortex_store.db._CORTEX_DB", db_path)
    op_result = _op_render_subgraph(root="decision:x", hops=1, neighbor_fidelity="full")
    app = create_app(db_path=str(db_path))
    client = TestClient(app)
    resp = client.get(
        "/subgraph/render",
        params={"root": "decision:x", "neighbor_fidelity": "full"},
    )
...
def test_v15_state_signals_block_omitted_when_empty():
    ...
    conn.commit()
    md = render_subgraph(conn, "decision:isolated", hops=1, **_FULL).rendered
...
def test_v15_provenance_flag_primary_source_backed():
    ...
    conn.commit()
    md = render_subgraph(conn, "decision:src", hops=1, **_FULL).rendered
...
def test_v15_provenance_flag_verbatim_quote_beats_uri():
    ...
    conn.commit()
    md = render_subgraph(conn, "decision:quo", hops=1, **_FULL).rendered
...
def test_v15_provenance_flag_derived_for_inference():
    ...
    conn.commit()
    md = render_subgraph(conn, "decision:inf", hops=1, **_FULL).rendered
...
def test_v15_provenance_flag_observation_class_no_flag():
    ...
    conn.commit()
    md = render_subgraph(conn, "decision:obs", hops=1, **_FULL).rendered
...
def test_v15_uri_shape_rejects_free_text_evidence():
    ...
    conn.commit()
    md = render_subgraph(conn, "decision:freetext", hops=1, **_FULL).rendered
    assert "[primary-source-backed]" not in md
...
def test_v15_uri_shape_accepts_absolute_path():
    ...
    conn.commit()
    md = render_subgraph(conn, "decision:abspath", hops=1, **_FULL).rendered
...
def test_v15_confidence_extraction_regex_unbroken():
    ...
    conn.commit()
    md = render_subgraph(conn, "decision:re", hops=1, **_FULL).rendered
...
def test_v15_provenance_flag_renders_on_related_entity_top_assertion():
    ...
    conn.commit()
    md = render_subgraph(conn, "decision:root", hops=1, **_FULL).rendered
    assert "**Top assertion:** [believed] [verbatim-quote] Child claim." in md
    conn.close()


# --- depth_aware neighbor fidelity ---


def test_depth_aware_hop1_omits_neighbor_description_and_top_assertion():
    conn = make_test_conn()
    add_entity(conn, "decision:root", "decision", "Root", "r")
    add_entity(
        conn, "todo:child", "todo", "Child", "Long neighbor description.", workflow_state="open"
    )
    add_edge(conn, "decision:root", "todo:child", "depends_on")
    add_assertion(
        conn,
    ...
    conn.commit()
    md = render_subgraph(
        conn, "decision:root", hops=1, neighbor_fidelity="depth_aware"
    ).rendered
    assert "Long neighbor description." not in md
    assert "**Top assertion:**" not in md.split("## Related Entities")[1]
    assert " assns · via " in md
    conn.close()


def test_depth_aware_hop2_sparse_block():
    conn = make_test_conn()
    add_entity(conn, "n0", "n", "N0")
    add_entity(conn, "n1", "n", "N1")
    add_entity(conn, "n2", "n", "N2")
    add_edge(conn, "n0", "n1", "depends_on")
    add_edge(conn, "n1", "n2", "depends_on")
    conn.commit()
    md = render_subgraph(conn, "n0", hops=2, neighbor_fidelity="depth_aware").rendered
    assert "**Edge:** depends_on" in md
    assert "N2 (`n2`)" in md
    conn.close()


def test_full_fidelity_regression_byte_identical_to_legacy():
    """AC#4: neighbor_fidelity=full preserves legacy markdown bytes."""
    conn = make_test_conn()
    seed_grokbuild_graph(conn)
    baseline = render_subgraph(conn, _ROOT, hops=1, neighbor_fidelity="full").rendered
    again = render_subgraph(conn, _ROOT, hops=1, neighbor_fidelity="full").rendered
    assert baseline == again
    assert "**Top assertion:**" in baseline or "Grokbuild V1" in baseline
    conn.close()


def test_depth_aware_default_is_not_full():
    conn = make_test_conn()
    add_entity(conn, "decision:root", "decision", "Root", "r")
    add_entity(conn, "todo:child", "todo", "Child", "neighbor desc", workflow_state="open")
    add_edge(conn, "decision:root", "todo:child", "depends_on")
    conn.commit()
    default_md = render_subgraph(conn, "decision:root", hops=1).rendered
    full_md = render_subgraph(conn, "decision:root", hops=1, neighbor_fidelity="full").rendered
    assert default_md != full_md
    assert "neighbor desc" not in default_md
    conn.close()
