...
_LIVE_DB = Path.home() / ".cortex" / "cortex.db"


def _live_db_ready() -> bool:
    """True when the operator substrate exists and has a migrated assertions table."""
...
_SKIP_LIVE = pytest.mark.skipif(
    not _live_db_ready(),
    reason=f"cortex live substrate not ready at {_LIVE_DB}",
)
...
def test_patch_decision_self_status_missing_workflow_col_is_non_fatal(
    ...
    legacy.executescript(
        """
        CREATE TABLE entities (id TEXT PRIMARY KEY, type TEXT NOT NULL, name TEXT NOT NULL, lifecycle TEXT);
