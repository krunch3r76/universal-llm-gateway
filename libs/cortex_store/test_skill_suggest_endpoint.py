...
def suggest_conn() -> sqlite3.Connection:
    ...
    _insert(
        ...
        source_uri="workspaces://universal-llm-gateway/.cursor/skills/cursor-only/SKILL.md",
    ...
    _insert(
        ...
        source_uri="workspaces://universal-llm-gateway/.cursor/skills/no-agents/SKILL.md",
        trigger_match_terms=["consult"],
...
@pytest.mark.asyncio
async def test_seat_gate_no_longer_filters_candidates(
    suggest_conn: sqlite3.Connection,
) -> None:
    """applicable_agents is metadata only — SQL seat gate retired."""
    body = await _call(
        {
            "agent": "claude-web",
            "loaded": [],
    ...
    # cursor-only has no trigger overlap with this context; gate retirement is SQL-level.
    assert all(s["slug"] != "cursor-only" for s in body["suggestions"])


@pytest.mark.asyncio
async def test_null_applicable_agents_candidates_eligible(
    suggest_conn: sqlite3.Connection,
) -> None:
    body = await _call(
        {
...
@pytest.mark.asyncio
async def test_suggestion_envelope_carries_source_uri_and_digest(
    suggest_conn: sqlite3.Connection,
) -> None:
    body = await _call(
        {
    ...
    item = next(s for s in body["suggestions"] if s["slug"] == "consult-routing")
    assert item["source_uri"] == "workspaces://universal-llm-gateway/.cursor/skills/consult-routing/SKILL.md"
...
@pytest.mark.asyncio
async def test_suggest_includes_rule_and_skill_types() -> None:
    conn = _conn()
    _insert_typed(
        conn,
        "rule:dispatch-shape",
    ...
    _insert_typed(
        conn,
        "skill:session-close-kernel",
        "skill",
        source_uri="workspaces://universal-llm-gateway/.cursor/skills/session-close-kernel/SKILL.md",
        trigger_match_terms=["session", "close"],
    )
    conn.commit()
    body = await _call(
        {
            "agent": "claude-web",
            "loaded": [],
            "conversation_context": "dispatch handoff session close",
        },
        conn,
    )
    slugs = {s["slug"] for s in body["suggestions"]}
    assert "dispatch-shape" in slugs
    assert "session-close-kernel" in slugs
