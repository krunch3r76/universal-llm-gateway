...
def test_session_debrief_token_format() -> None:
    from cortex_store.session_close_debrief import session_debrief_token

    assert (
        session_debrief_token("cursor-2026-06-02-120000-a01")
        == "[session:cursor-2026-06-02-120000-a01]"
    )


def test_compose_debrief_body_includes_token_and_fields() -> None:
    from cortex_store.session_close_debrief import compose_debrief_body

    body = compose_debrief_body(
        session_id="cursor-2026-06-02-120000-a01",
        agent="cursor",
    ...
    assert "[session:cursor-2026-06-02-120000-a01]" in body
...
def test_attempt_debrief_skips_existing() -> None:
    from cortex_store.session_close_debrief import (
        attempt_session_close_debrief,
    )
    ...
    with (
        ...
        patch(
            "cortex_store.session_close_debrief._find_existing_debrief",
            return_value=799,
        ),
    ):
        outcome = attempt_session_close_debrief(
...
def test_attempt_debrief_posts_when_no_existing() -> None:
    from cortex_store.session_close_debrief import attempt_session_close_debrief
...
def test_attempt_debrief_failed_returns_body() -> None:
    from cortex_store.session_close_debrief import attempt_session_close_debrief
    ...
    assert "[session:cursor-2026-06-02-120000-a01]" in outcome.debrief_body
...
def test_idempotent_reclose_retries_debrief(
    ...
    from cortex_store.session_close_debrief import DebriefOutcome
    from cortex_store.test_session_close_handoff import _session_summary

    with patch(
        "cortex_store.routes.session_close_persist.attempt_session_close_debrief"
    ) as mock_debrief:
