...
from event_store.delivery_audit_schema import (
    _PROVIDER_AFFORDANCE_COLUMN_DEFS,
    _TOKEN_LOCALITY_COLUMN_DEFS,
)

TOKEN_LOCALITY_COLUMN_NAMES = [name for name, _ in _TOKEN_LOCALITY_COLUMN_DEFS]
PROVIDER_AFFORDANCE_COLUMN_NAMES = [
    name for name, _ in _PROVIDER_AFFORDANCE_COLUMN_DEFS
]
...
def _insert_child(
    ...
    *,
    ...
    provider_native_name: str | None = None,
    capability_class: str | None = None,
    support_mode: str | None = None,
    effect_axes: str | None = None,
    authority_delta: str | None = None,
    capability_event_type: str | None = None,
    applicability_basis: str | None = None,
    provider: str | None = None,
    model: str | None = None,
    recipient_agent: str | None = None,
    rendered_content_hash: str | None = None,
    audit_checker: str | None = None,
    audit_checker_version: str | None = None,
    audit_evidence_uris: str | None = None,
) -> None:
    now = _now()
    ...
    conn.execute(
        ...
        INSERT INTO delivered_artifacts (
            ...
            capability_key, provider_native_name, capability_class, support_mode,
            effect_axes, authority_delta, capability_event_type,
            applicability_basis, provider, model, recipient_agent,
            rendered_content_hash, audit_checker, audit_checker_version,
            audit_evidence_uris, created_at, updated_at
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """,
        (
            new_artifact_record_id(),
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "workspaces://universal-llm-gateway/.cursor/skills/"
            "consult-routing/SKILL.md#Implement lane"
        ),
...
def test_ensure_schema_adds_provider_affordance_columns(registry_db: Path) -> None:
    with connect(registry_db) as conn:
        columns = _column_names(conn, "delivered_artifacts")
    for name in PROVIDER_AFFORDANCE_COLUMN_NAMES:
        assert name in columns
...
def _complete_provider_affordance_row(**overrides: object) -> dict[str, object]:
    row = {
        "artifact_class": "provider_affordance_surface",
        "artifact_id": "openai/gpt-5/advisor",
        "provider": "openai",
        "model": "gpt-5",
        "recipient_agent": "cursor-sdk",
        "affordance_kind": "advisor",
        "capability_key": "provider_native.advisor",
        "provider_native_name": "advisor",
        "capability_class": "provider_native",
        "support_mode": "native",
        "effect_axes": '{"see":["routing"],"decide":["lane"],"do":[]}',
        "authority_delta": "advisory_only",
        "capability_event_type": "delivered",
        "applicability_basis": '{"for_agent":"cursor","seat":"cursor"}',
        "rendered_content_hash": "abc123",
        "audit_checker": "provider-affordance-gate",
        "audit_checker_version": "v1",
        "audit_evidence_uris": '["cortex://evidence/1"]',
...
def test_provider_affordance_surface_round_trips_all_fields(registry_db: Path) -> None:
    audit_id = new_audit_id()
    fields = {
        "capability_key": "provider_native.advisor",
        "provider_native_name": "advisor",
    ...
    with connect(registry_db) as conn:
        ...
        row = conn.execute(
            f"""
            SELECT {", ".join(PROVIDER_AFFORDANCE_COLUMN_NAMES)}
            FROM delivered_artifacts WHERE audit_id = ?
            """,
            (audit_id,),
        ).fetchone()
    assert row is not None
    assert dict(zip(PROVIDER_AFFORDANCE_COLUMN_NAMES, row, strict=True)) == fields


def test_derive_child_audit_status_audited_clean_when_complete() -> None:
    row = _complete_provider_affordance_row()
    assert (
        derive_child_audit_status(row, audit_policy_version="policy-v1")
        == "audited-clean"
    )


@pytest.mark.parametrize("missing_field", ["provider", "support_mode", "effect_axes"])
def test_derive_child_audit_status_unaudited_when_missing_clean_evidence(
    missing_field: str,
) -> None:
    row = _complete_provider_affordance_row(**{missing_field: None})
    assert (
        derive_child_audit_status(row, audit_policy_version="policy-v1") == "unaudited"
    )


def test_derive_child_audit_status_unaudited_not_not_applicable_when_incomplete() -> (
    None
):
    row = _complete_provider_affordance_row(
        support_mode=None, audit_status="not-applicable"
    )
    assert (
        derive_child_audit_status(row, audit_policy_version="policy-v1") == "unaudited"
    )
...
def test_validate_provider_affordance_rejects_invalid_support_mode() -> None:
    row = _complete_provider_affordance_row(support_mode="bogus")
    with pytest.raises(ValueError, match="unknown support_mode"):
        validate_provider_affordance_surface_fields(row)


def test_validate_provider_affordance_noop_for_other_artifact_classes() -> None:
    validate_provider_affordance_surface_fields(
        {"artifact_class": "http_skill_body", "support_mode": "bogus"}
    )
