...
waking that tab (CreateGoal + ``notify_on_output``) after RETIRED.
Specimen: 11912 hop 2026-09-21 — land ``7484bed2-…``, loop killed by
hand, goal still ran 5m45s because the skill forbade UpdateGoal on hop.

Pollers stay: the successor ARM line is ``watch-supervise.sh tail
--label``. ``--forever`` debug tails stay. UpdateGoal is Cursor-native
— the skill obligates ``complete`` as tab-goal release (¬ house close).
"""
...
def retire_departing_tab(
    ...
    *,
    ...
    """Kill this tab's loop + tails and free the ``ide:`` seat.

    Does not stop pollers (successor ARM is ``tail``). Does not UpdateGoal.
    """
