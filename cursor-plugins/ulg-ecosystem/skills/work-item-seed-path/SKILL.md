...
#### G-row `sdk_mode` ladder (conductor-owned — BINDING default)

After S6 spawn the **conductor** stamps `sdk_mode` on nested cursor-sdk legs.
CDP rows (G1/G2/G4/G6) are out of scope — they use `team_dispatch(model=cdp/…)`.
