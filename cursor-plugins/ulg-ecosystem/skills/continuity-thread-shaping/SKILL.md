...
| Resume | Cortex-house order. Speaking it? Slot order = `operator-posture` Rule 3; fill sources → `resume-fill.md`. The imprint line is the only slot this skill originates, and it follows the picture. IDE tab `{n} {slug}` |
...
Checkpoint happy path is `checkpoint-discipline` § Pipeline CHECKPOINT (`pre_consolidate=false` + grok tip author).
Card patch, sketchboard / continuity-doc fold, and `consolidate-continuity` are **off** that path.
