...
**conductor score** = conductor packet (admit file) + scoreboard (G-rows). ¬ shorten to **score**. **Admit package** retired. Mission Composer (hop) ≠ Conductor admit; fire via `runbook:score-composer-author` with **omit `model=`** → `workflows.auto_judgment` (`route_policy.yaml`). ¬ hard-pin experimental `grok-4.7`; operator-named Grok only → `cursor/grok-4.5` high. Sketch ≠ path-sim / `/layer` architecture and ≠ a casual doodle — it is the symphony sketchbooks: bind themes, movements, in/out.
