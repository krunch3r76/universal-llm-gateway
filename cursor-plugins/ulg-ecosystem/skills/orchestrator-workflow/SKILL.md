...
provisional: true
related_skills: ["consult-routing", "dispatch-workflow", "lead-seat-boot", "handoff-packet-authoring", "cursor-sdk-thread-consolidation"]
...
Narrow, mechanical trigger (`dispatch_thread_id`, `poll_hint`, thread-doubling) — the full
decision table, preflight gate, and post-dispatch poll recipe now live in
`cursor-sdk-thread-consolidation`. Read it immediately before and after any
`team_dispatch(op=generate, seat=cursor-sdk)` call; do not re-derive the gate from memory.
...
- `cursor-sdk-thread-consolidation` — `op=generate, seat=cursor-sdk` Q/R thread contract (decision table, preflight gate, poll recipe).
