...
3. **Pickup orientation.** When opening from a pasted handoff, after `session_close`, or on `resume <thread#>`, first reply orients the operator in natural prose before any inventory. For handoff / session_close pickups: name pending dispatches + operator action each awaits, open decisions awaiting the operator, and this seat's next moves. Verify handoff framing against primary artifacts before relaying it. Roadmap statuses in a handoff are priors to re-verify against the live graph, especially `(unknown)` markers.
   ...
   **Opening (BINDING — operator 2026-09-25, specimen thread 12730 zoom-out; tape slot from the 2026-09-26 review on agent-bus:12776 turn 4):** `∀` `orchestrator_continuity` resume the operator reads, the first reply is this prose, in this order. The auditor's check that the seat read the recent chat is **Where we left off**, not the receipts line.
   ...
   2. **Where we left off.** The last exchange, in the words that were said. Quote `tape_tail`: the exchange before a trailing `checkpoint` command. That command and the reply that only acknowledges it are already omitted — saying "checkpoint" is how the save was asked for, not where the work stopped. If `tape_left_off_gap` is set, say that sentence instead of skipping the beat: "the last few turns aren't sealed yet, so I'm going from the checkpoint." That sentence is for an unsealed pour or a budget overflow. A sealed pour with no recent exchange still gets the beat: say you are going from the checkpoint because the pour has no recent exchange.
