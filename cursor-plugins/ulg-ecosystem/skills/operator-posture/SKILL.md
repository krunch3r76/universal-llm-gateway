...
2. **Translate dispatches.** Any turn that fires `team_dispatch` or authors a handoff closes by saying: what was dispatched, to whom, executor, autonomous vs operator action, and how/when results return. For `op=generate`, state server-derived `resolved_model` **and a one-line tier rationale** whenever the pick is an explicit judgment-tier (T1+) model — e.g. `workflows.auto_judgment → cursor/composer-2.5 (omit model=; route_policy register)`. Explicit Grok pin only when named: `cursor/grok-4.5` high (¬ experimental `grok-4.6`). For `op=handoff`, use server `recommended_executor` / `recommended_review`; Composer is the stated implement default. Do **not** infer executor from packet prose, subject, or model-family intuition. Echoing `push_reminder` verbatim is failure. **Harvest briefing (Lane B):** when closeout carries `branch` + `head_sha` and land is still owed, state in one line: ``branch=<name> head=<sha> land owed`` (or ``landed`` when discharged).
   ...
   **Attended in-flight (binding — operator 2026-09-01; audience split 2026-09-04):** When the dispatch will outlive this turn, arm a cheap wake and exit — do not hold this turn on `wait`. Arm recipe SoT: **`runbook:bus-consult-watcher`**. Two audiences — do not conflate:

   | Audience | Default path | SMS/`--page`? |
   |---|---|---|
   | **This seat** (in-window agent harvest) | `watch-supervise.sh start` (detached poller) + IDE `tail` + `notify_on_output` on `consult complete`; `--no-page` — SoT `runbook:bus-consult-watcher` | **No** |
   | **Human** left IDE / wants pager | Optional tmux **pane** in existing session `0` (¬ new session) **with** page enabled | **Yes** — human-notification only |

   ¬ arm tmux for the agent's own wake — that pattern is reserved for human out-of-IDE notification.    Cheap = watcher above + lean heartbeat (`loop` skill local event path). Costly = short-cadence `/loop` that re-runs a full agent turn, or holding this turn on `wait`. Complements a:31104 (liaison does not gate harvest on an operator check utterance). ¬ the uncommissioned always-on liaison (a:31024). **Liaison HARD STOP (a:32226):** when a stronger-model gate (G4 Skeptic / gated after-ship) returns CDP stall, empty FAILED, or no harvest — **stop** mission progression past that gate; ¬ executive-extend the score to nest implement / land / DONE; ¬ reframe as transport debt while continuing. Resume only after CDP harvest or operator-explicit seat rebind (`conductor` skill).

   **Watcher mis-arm imprint (binding):** wrong script family, bad `after_turn`, invalid flags, or false `complete` on a dispatch arm ⇒ **same turn** before close: informing `friction()` per `friction-review` § Informing frictions (`owner=agent_skill:agent-bus-discipline` for cursor-sdk closeout; `runbook:bus-consult-watcher` assert pin) + **¬** mint `todo:`. Accumulate with same-class cluster; one todo only at commission/consolidation triage.

   **EXIT / idle announce (binding — a:32324 · a:32323 · a:32326 · operator 2026-09-04):** After a watcher harvest, if the tip shows `NEXT_ADMIT: none` / mission complete / `PARKED` / no live cursor-sdk on the watched work — **say that in the operator reply in one hard line** before any residual: conductor/worker **exited or idle**, expect silence, what hygiene (if any) is still open. Soft-pedaling as “stamp may be owed” while the seat looks dead is a liaison failure. Watcher wake ≠ awareness; the harvest turn must translate exit into human-visible posture. Substrate stall-pop still queued on a:32324 (was a:32318).
