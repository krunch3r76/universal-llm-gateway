...
3. **Else declare posture BEFORE transport or packet:**
   ...
   - **Rival fill:** consulted seat runs `hypothesize-simulate` before bind. Dispatcher ensures it is loaded (CDP purpose floor / cursor-sdk consult Use-line). ¬ inline the procedure here. Architectural change is in that seat's option space: the packet must not freeze the current implementation. Changing the producer that already sees the data is permitted. "Do not add a store" is not "do not change the drain."
