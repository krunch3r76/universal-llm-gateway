...
| **M-Densify** | Composer enumerate → **`cdp/fable` bind** (`seat=cursor-sdk` + `contract=none`) | `tmp/modularize-plans/{name}.md` — MODULES + IMPLEMENTATION GUIDE + `files_expected` |
...
```python
# 1. Composer enumerate (omit model=, contract=none)
# 2. team_dispatch(model=cdp/fable, contract=none, …) bind
team_dispatch(
  op="generate",
  seat="cursor-sdk",
  contract="none",
