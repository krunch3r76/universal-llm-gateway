...
description: "Cursor model economics — $/M rates, conductor T0–T3 tier table, Sonnet 5 vs Opus/GPT context billing, Grok effort policy, Auto/Router entitlement. CDP + Cursor shared_sync annex."
---
...
| **T3** | `cursor/claude-opus-5` | `low`→`high`; `xhigh`/`max` need standing trigger | Invariant-touching bind — inform-then-proceed |
...
## Grok effort — capability vs policy

| Layer | Grok 4.6 effort |
|---|---|
| **Live capability** | Through **`xhigh`** (card + API accept) |
| **Standing policy** | Auto / conductor / nested investigate default **cap at `high`** (reliability) — do **not** widen the Auto effort clamp |
