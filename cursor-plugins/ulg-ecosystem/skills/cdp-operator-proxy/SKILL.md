...
| **Human operator (Kaywan)** | Only when **explicitly declared** in chat — e.g. `I am the operator`, `human operator here`, `Kaywan operating`, or equivalent unambiguous declaration | ¬ inferred from Cowork session, product Ask UI, conversational register, or silence |

**Invariant 0:** `default(operator) = model_seat` · `human_operator ⇔ explicit_declaration`. Absent declaration: assume **non-human** — this seat is operator; Kaywan is the **human principal** for awareness / interrupt pager and true operator-only gates, not the active DIRECTIVE author.
...
**Consequences:** Cowork CSE presence alone does not declare human operator. **In-session `notify` carve-out** (inv 22) applies only when Kaywan has declared human operator **and** is in *this* Cowork CSE. Blocked→ask step 3 (Kaywan ping) is for **operator-only** forks — not routine routing or inferred human presence. Human declaration that explicitly hands off the seat (`you are not operator`, `I'm driving`) binds for that stream; otherwise seat authority stands.
...
8. `tool_absent(life) ⇏ Kaywan_gate` — missing life-MCP tool that **code seat holds** ⇒ `agent_bus.request` to cursor; ¬ park on Kaywan in prose (friction class: 5901 reload residual)
...
10. **`cursor/claude-opus-5` — escalation option, inform Kaywan.** When live checkout browse or premium cursor-substrate judgment warrants it, fire with **inform-then-proceed** (one legible NOTE/chat line: model + why) — **¬ default wait for Kaywan approval**. Still wait when Kaywan has paused premium spend, or for extreme knobs (`xhigh`/`max`) without a standing trigger. ¬ `anthropic/*` API. Compose: `lean-context-dispatch-first` · `anthropic-dispatch-authorization`.
...
| 3 | **Cowork Ask / push Kaywan** — one question + recommended answer (he is pinged) — **operator-only** forks only (invariant 0); ¬ routine routing because Cowork chat might be human |

**Code-seat ops (always step 2 — never step 3 alone):** `manage` / `charter_reload` / manage quit-start / service lifecycle / tree contradiction / any tool on vortex-code but not life. **Service restart from this seat:** `agent_bus.request` with `contract: propagate` (drain-gated — not tier-M `execute` + `manage.*`). (`charter_reload` restarts the tick loop and returns `count=0` — it does **not** re-import modules, so charter-runner code changes need a manage quit/start.) Prefer conferring with cursor (later API seats) for **operational** “what’s optimal next”; Kaywan gates stay for true operator judgment (proceed / implement / irreversible human action / **Authorize-triggers** when the product prompts — invariant 21).
...
2. **kaywan_gate** — `TYPE: RESIDUE` + residual-imprint on the matter entity **and** Kaywan pinged (chat if in-session; pager if away). Kaywan is always pingable for human gates — ¬ park silently because “tonight is over.”

**Wake path (BINDING — fail-closed):** `commissioned, in flight` alone is an **invalid close state**. Every outstanding item at mission/episode close must carry one of: `collector: <seat that will harvest>` · `followup: <how/when>` · `charter_enrolled: <root>` · `kaywan_gate: <reason>`. ¬ per-mission babysitter / ad-hoc watchdog as the remedy — structural wake path only.

```
∀ DISPOSITION(mission_close ∨ episode_close) ∨ TYPE: MISSION_CLOSEOUT:
  residual_set = ∅
  ∨ ∀ r ∈ residual_set: (commissioned(r) ∧ wake_path(r)) ∨ kaywan_gate(r)
¬ “residuals in the sidecar” + “No action required” while Auto-runnable work remains
¬ “commissioned, in flight” with no collector / followup / enrollment / kaywan_gate
```
...
| DISPOSITION names residuals / “not commissioning tonight” with no Auto request | Residual-commission gate: DIRECTIVE to cursor-auto **or** kaywan_gate + ping |
| DISPOSITION names residuals / “sidecar” / “not commissioning tonight” with no Auto request | Residual-commission gate: DIRECTIVE to cursor-auto **or** kaywan_gate + ping |
...
| Silent stop / “until you tell me” with no Ask/push | Blocked → ask ladder (observe → cursor → Kaywan ping) |
| “`manage` not on life ⇒ Kaywan gets reload/liveness” | `agent_bus.request` with `contract: propagate` (or cursor `manage` on code seat); Ask Kaywan only if cursor cannot |
| `contract: execute` + `tool_op: manage.sync_restart` | Denied at tier-M manifest — use `contract: propagate` |
| Operational choice defaulted to Kaywan gate | Confer with cursor first; Kaywan for proceed/implement/irreversible only |
...
| Parking a CDP-Opus → Fable escalation on Kaywan | Opus escalates outward on its own judgment; only `cursor/claude-opus-5` is normally his gate — ping via the Cowork "question" prompt |
...
| Cowork CSE open / chatty tone ⇒ Kaywan is operator | Invariant 0: model seat is operator until human **explicitly declares** |
| Routine blocked→ask escalates to Kaywan without operator-only fork | Steps 1–2 (observe → cursor); Kaywan ping only for credentials / irreversible human act / genuine *what we want* ambiguity |
