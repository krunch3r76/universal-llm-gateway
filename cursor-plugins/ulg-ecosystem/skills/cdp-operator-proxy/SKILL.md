...
is *confirmed* (registration / `chat_url` observed), or (c) true mission/episode close
...
Awareness class — the one pager that earns length. Layman prose throughout: ¬ slugs, thread ids,
SHAs, paths, contract tokens, closeout shape; metrics in plain language ("eleven items to two").
Subject must **not** say `COME TO IDE`.
...
| A ticket/slug dump as the "debrief" on the pager | Layman prose — vision, architecture, what he can trust now |
