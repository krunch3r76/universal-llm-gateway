...
3. **Arm-and-re-arm (BINDING when step 2 says Leg, or when you park with work still in flight):** before this turn's tool loop ends, arm or refresh a **self-scheduled wake** into *this* session — Cowork **Monitor** heartbeat (cadence ≤ ~240s while work is live) and/or **`send_later`** (same-session user turn). Re-arm at every subsequent turn boundary while the lane has in-flight work or an open residual that this CSE still owns. This duty sits **next to** the going-quiet rule at the same deciding moment: reading quiet≡stop without arming the wake is incomplete. State the armed mechanism + interval (or trigger id) on the lane when you park. Wake does not prevent the stop; it bounds silence (inv 30 mechanisability).
...
**Concrete:** just landed a roadmap row, feel the pull to summarise and rest ⇒ answer is Leg, stream continues — report, arm/re-arm Monitor or `send_later`, then poll/harvest/commission the next act. No interpretation step.
