...
| 5 | `cursor/gpt-5.6-terra` (default when bindable) or `cdp/fable` | `light-bounded` | **Independent check** — falsify hop 4's load-bearing premises. If terra returns `model_pin_refused` (bindable cursor-sdk set is composer-2.5 / grok-4.6 / opus-5; CDP+Fable via `escalation=`), **fall through to `cdp/fable` same turn** — do not leave hop 5 undischarged, and update peer disclosure (inv 36 standing claim). |
...
| investigate | grok-4.6 — reasoner; `contract: investigate` (inv 28) |
