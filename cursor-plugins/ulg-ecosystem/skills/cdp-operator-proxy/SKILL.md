...
**CSE lifetime (BINDING):** the **lane + `chat_url` identity** live until (a) human escalation, (b) continuity after successor confirmed (inv 30), or (c) MISSION_CLOSEOUT + debrief. ¬ kill identity on poller timeout. `wall_clock_exceeded` ⇒ reattach. `max_wall_s` = since last fingerprint delta. The **generate stream** is a different lifetime — PARK / stand-down / Stop-click may end the stream without ending the CSE or the private bus lane (`decision:cse-tab-decoupled-from-session`).
...
25. **Bus recency ≠ liveness** — fleet gate attestation authoritative when `fleet_gate_applied: true`; on dispatch lanes `wait.producer.state` (O14 D2, pinned `execution_id`) beats thread recency / unrelated closeout subjects.
