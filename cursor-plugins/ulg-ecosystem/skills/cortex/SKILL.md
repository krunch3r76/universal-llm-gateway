...
| `entity_create` | Create entity. **`person` / `organization`:** omit `id` (server mints opaque ULID slug); supplying `id` → 422 `entity_id_minted_type`. **Designated types:** `id` required — may be `type:slug` or bare slug (server canonicalizes); mismatched type-prefix → 422. Duplicate display name on minted types → 409 unless `duplicate_name_ok=true`. Duplicate id → 409. Option-C traits (`confidence_band`, `lifecycle`, `adoption`) **not** settable at create — derived at birth; set via `entity_update`. `status` synthesized on read — do not pass. `workflow_state` auto-filled to type's `initial_state` when omitted. Auditor-validatability: confirmed-band entities need ≥1 `confidence='confirmed'` assertion citing source (session_close audit-gate flags gaps). |
