...
| `friction` | Log tool/schema/boot friction — **observation only, NOT ticket channel**. `owner`: bare slug → `service:{slug}` or full `service:`/`agent_skill:`/`ai_agent:` id (non-service owners must exist). `service=` back-compat alias. Optional `confidence` (`confirmed`/`believed`/`suspected`/`hypothesized`) is **honoured**; invalid values are rejected (never silently downgraded). Omitted → `hypothesized`/0.5. Categories: `tool_error`, `tool_mismatch`, `tool_absent`, `schema_gap`, `boot_drift`, `lesson_gap`, `lesson_conflict`, `stale_context`, `doc_drift`, `protocol`. Actionable rows → codified bug cycle (investigate → execute); see `agent_skill:friction-review`. |
