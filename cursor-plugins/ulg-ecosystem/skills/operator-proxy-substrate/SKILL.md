...
| # | Step | Evidence |
|---|---|---|
| Q | Withdraw a still-queued predecessor (`queue_withdraw`) before it claims | `method: queue_withdraw`, `terminal_status=displaced_queued`; job never claims |
...
Queued predecessors take **Q + 2** (no process, no writes — skip 1 and 3). Claimed predecessors take 1–4 (`pre_register_live_run` skips process-stop but still marks displacement).
