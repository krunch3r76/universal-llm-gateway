...
**Post-idle cadence (a:26348):** once `turn_idle` **or** `stop=true`, re-poll **≤5–10s** until `archive_uri` | verified `content_proof` | `failed`+`stall_stage`. Chat may skip `content_proof_uri` — do not wait for a rung that never comes. Forbidden: multi-minute sleeps between polls. Poller-transport loss retains (no Stop-click) and publishes `cdp UNVERIFIED` + `chat_url` when a `cse_` was observed; `cdp FAILED` + `observer_unverified` + no `chat_url` = satellite lost before any CSE address — re-dispatch.
...
**Anti-pattern:** opening a new CDP consult via any leftover `project_ask` recipe. Product is `team_dispatch(model=cdp/…)` on code. On life, `life_dispatch` is life→cursor operator-proxy only — not a life conversation and not claude.ai→claude.ai chat. Correspondence stays in this CSE (`life-session-engagement` / `outbound-voice-spec`). `cursor_request` when the work order does not need a new CSE.
