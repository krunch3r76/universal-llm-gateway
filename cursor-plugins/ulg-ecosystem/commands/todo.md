...
Rich todos carry `source_uri: cortex://notes/system/specs/{slug}.md` (RAG-indexed in the
`todo_specs` scope).
...
**Grouping**: when ≥2 leaf todos form one bounded, closable arc, group them under
a `task:` entity via `child_of` (members ordered by **steps** — todo ordering /
`depends_on` — not phases; see `@awareness_ws.mdc` and
`agent_skill:task-grouping-discipline`); a larger umbrella over multiple arcs/plans
is a `project:` (spec + graph edges); **ordered phases** (`plan_phase:`) apply only
to a `plan:`. Not a master-`todo:` — a `todo:` is always a **leaf**. (Only the `task-seed`/`task-close`
*workflow* was retired 2026-06-04 — the `task:` entity type is live,
`decision:task-subsystem-retired`.) See `cortex://notes/system/specs/agent-workflow-parity.md`.
...
**Admission provenance (Step 3 — capture only, no reroute):** at pickup, derive
`source_ref=todo:{slug}` (canonical `todo:` grammar). Stash it in session metadata
for close — pass `source_ref` and `source_ref_derivation="ide-todo-pickup"` on
`session_close`. Provenance capture only — IDE execution is **not rerouted**
through the Stargate admission bridge.
...
2. Write a stub spec at `cortex://notes/system/specs/{slug}.md` (Problem / Scope / Acceptance
...
| "add X" | If codework mint needs recon/architecture-order/backlog choice → `/work-item-seed` (skill `work-item-seed-path`) first. Else apply rich-seed contract (§3b) by default. Write stub spec + edges atomically. Backlog/deferred intent → simple entity + `backlog=true` (or seed-path S4 backlog-park). Slug per `@plan-slug-coherence_ws`. |
