...
4. `team_dispatch(op=generate, model=cdp/opus-5, contract=none, sidecar_ref=cortex://…, dispatch_thread_id=…)` — wait via `poll_hint` / `agent_bus.wait` until `archive_uri`. **Escape only:** CLI `scripts/cortex/claude-ai-sync-jupiter project-ask` when `team_dispatch` CDP unavailable
