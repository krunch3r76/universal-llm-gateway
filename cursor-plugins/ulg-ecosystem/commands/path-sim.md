...
7. **Run R-after yourself (lead-owned, default-on)** — after Stage-B ship: `/work-item-review todo:{slug}` on **`seat=cursor-sdk, model=cursor/grok-4.5, contract=light-bounded`** (skill § R positions substrate split — ¬ web-anthropic CDP, ¬ `xai/grok-*` artisan). Acceptance ledger · drift · docstring scan · event-instrumentation challenge. Same closed skip set as R-admit. **¬ skip because "implement looked clean".** Apply REVISE (direct patch or follow-up todo) before closeout.
...
| Skip R-after after path-sim implement (no closed-set evidence) | Fire `/work-item-review` · `cursor/grok-4.5` — delivery half of external R |
...
Use the `path-sim` skill · `cheap-recon-before-escalation` skill · `handoff-packet-authoring` skill (packets + Gate-2) · `implement-todo` skill (closeout) · `claude-ai-cdp-navigation` skill when R-admit runs · `event-instrumentation-discipline` skill (Stage-B + R-after) · `/work-item-review` for R-after (`cursor/grok-4.5`)
