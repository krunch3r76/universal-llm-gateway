...
- `projects/.cursor/rules/handoff-dispatchers.mdc` — `team-generate` per-pass
  dispatch shape; `agent-skills/consult-routing.md` for surface choice
- `/diff-review` — task adapter (scope discovery, file list, SLOC gate,
  review manifest, 5 review categories, output format refinement)

`/diff-review-mcp` adds: agent_bus thread continuity across passes,
adversarial cross-model alternation, programmatic convergence detection,
hard pass cap.

This command exists separately from `/diff-review team-generate` because the
...
## Differences from `/diff-review team-generate`

| Aspect | `/diff-review team-generate` | `/diff-review-mcp` |
|---|---|---|
...
For simpler reviews, prefer `/diff-review team-generate`. Use this command when
...
- The base `/diff-review team-generate` (single-shot) feels insufficient

## Invocation

```
/diff-review-mcp [model | adversarial] [path] [since <git-ref>] [passes:<N>]
```

| Argument | Meaning |
|---|---|
| omitted or `openai` | All passes use `openai/gpt-5.5` |
| `gpt-5.5` / `gpt-5` | All passes use `openai/gpt-5.5` |
| `gpt-5.4` | All passes use `openai/gpt-5.4` |
...
| `adversarial` | Alternate `openai/gpt-5.5` ↔ `google/gemini-3-pro-preview` per pass (pass 1 = openai) |
...
```
/diff-review-mcp
/diff-review-mcp gemini
/diff-review-mcp adversarial
/diff-review-mcp adversarial passes:6
/diff-review-mcp libs/transport_utils/client_factory.py since HEAD~1
/diff-review-mcp adversarial libs/transport_utils/ passes:4
```
...
- `adversarial` → `MODE_ADVERSARIAL = True`. Rotation:
  `["openai/gpt-5.5", "google/gemini-3-pro-preview"]`; pass `K` uses index
  `(K - 1) % 2`. Cannot combine with explicit model token.
- `passes:N` → `PASS_CAP = N`. Validate `1 <= N <= 8`. Default 4.

Defaults: `MODE_ADVERSARIAL = False`, `REVIEW_MODEL = "openai/gpt-5.5"`,
`PASS_CAP = 4`.
...
Run `/diff-review` steps 1, 1a, and **2m** (MCP `<invariants>` — cortex skill
refs + per-task narrowing; see `architecture-handoff-protocol.mdc` § Block 2).
Outputs needed:
...
- Six packet blocks (`<scope>`, `<excluded>`, `<invariants>`,
  ...
  `<invariants>` MUST name `agent-skills/architecture-invariants.md` and
  `agent-skills/ulg-architecture.md` (not a re-inlined universal layer).
...
THREAD_RESP = agent_bus(tool="post", arguments=json.dumps({
    "slug": f"diff-review-mcp-{BRANCH}-{HEAD_SHA}",
    "to": "frontier-reviewer",
    "from_agent": "cursor",
    "subject": f"Diff review (mcp dialectic): {BRANCH} @ {HEAD_SHA}",
    "tags": [f"project:{REPO_NAME}", "type:review", "agent:cursor", "mode:diff-review-mcp"],
...
0. fs(sandbox="cortex", op="read", path="agent-skills/architecture-invariants.md")
   ...
   — read BEFORE forming findings (canonical invariant layers; packet
   `<invariants>` is per-task narrowing only).
...
2. Investigate via fs / cortex / observability / rag (per
   `projects/.cursor/rules/handoff-dispatchers.mdc` § "team-generate" and `/diff-review` § 4m
   `<mcp_capabilities>` — item 0 is the skill reads above).
...
3. For any finding that changes event signals, event payloads, API surfaces, or
   runtime coordination contracts, state whether `docs/event-contracts.md` must
   be updated. Its catalog regions are generated from `@event_factory` call
   sites (regenerate via `gen-event-catalog`, never hand-edit inside
   `<!-- GENERATED -->` markers); the prose outside the markers is curated.
...
```
PASS_MODEL = (
    ["openai/gpt-5.5", "google/gemini-3-pro-preview"][(PASS - 1) % 2]
...
- `<invariants>` block (from `/diff-review` step 2m — skill refs + narrowing)
- `<mcp_capabilities>` block — paste `/diff-review` § 4m adapted block (item 0:
  architecture skills) PLUS the agent_bus fetch+reply directive from step 3's
  thread-init body
- `<output_format>` block — `/diff-review` § 4m adapted `<output_format>`,
  augmented to require BOTH a thread reply AND inline return
...
Per `projects/.cursor/rules/handoff-dispatchers.mdc` § "team-generate" / "Dispatch":
...
Failure handling per `projects/.cursor/rules/handoff-dispatchers.mdc` § "team-generate" / "Failure
...
Write artifact at `tmp/reviews/${BRANCH//\//-}-diff-review-mcp.md` with the
...
Source-of-truth per `projects/.cursor/rules/handoff-dispatchers.mdc`: agent_bus thread.

## Diff-Review-MCP Specifics
...
- **Architecture skill priming** — `<invariants>` uses cortex skill refs
  (`architecture-invariants`, `ulg-architecture`); reviewer item 0 in
  `<mcp_capabilities>` matches `/diff-review` § 4m
...
- Always audit docs/contracts before closure; in `docs/event-contracts.md` the
  catalog regions are generated (regenerate via `gen-event-catalog`, never
...
- All other rules inherited from `architecture-handoff-protocol.mdc`,
  `projects/.cursor/rules/handoff-dispatchers.mdc`, and `/diff-review`
