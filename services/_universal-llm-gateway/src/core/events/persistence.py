"""
Event Persistence - SQLite-based event storage for historical analysis.

Provides EventStore class for storing and querying events with:
...
class EventStore:
    """
    SQLite-based event storage for historical analysis.

    Stores events in a SQLite database with automatic schema management,
    cleanup policies, and query capabilities.

    Example:
        store = EventStore("/tmp/llm_gateway_events.db")

        # Store events
        store.store_event(ModelLoaded(...))

        # Query events
        recent_events = store.query_events(since=time.time() - 3600)  # Last hour
        model_events = store.query_events(event_type="ModelLoaded")

        # Cleanup old events
        store.cleanup_old_events(max_age_days=7)
    """
    ...
    def cleanup_old_events(self, max_age_days: int) -> int:
        """
    ...
    def _auto_cleanup(self):
        ...
        # Only cleanup periodically (check every 1000 events)
        try:
            with sqlite3.connect(self.db_path) as conn:
                cursor = conn.execute("SELECT COUNT(*) FROM events")
                count = cursor.fetchone()[0]

            # Cleanup every 1000 events
            if count % 1000 == 0:
                self.cleanup_old_events(self.auto_cleanup_days)

        except Exception as e:
            logger.debug(f"Auto-cleanup check failed: {e}")

    def _enforce_max_events(self):
        """Enforce maximum events limit by deleting oldest"""
    ...
    def close(self):
        """Close database connections (SQLite auto-manages connections)"""
        logger.info("📦 EventStore closed")


class EventStoreSubscriber:
    """
    Subscriber that automatically stores events in EventStore.
    ...
    Example:
        event_bus = EventBus()
        event_store = EventStore("/tmp/events.db")
    ...
    def __init__(
        ...
        Args:
            ...
            event_store: EventStore to store events in
        ...
        if event_signals:
            for signal in event_signals:
                ...
                logger.debug(f"EventStoreSubscriber: Subscribed to signal '{signal}'")
        else:
            logger.warning(
                "EventStoreSubscriber: No event_signals specified, manual subscription required"
            )
