...
remote-MCP boundary. Two structurally distinct conditions:
...
The ``misconfigured`` branch precedes a terminal ``pipeline_execution_failed``
and includes ``model_entity_id`` for post-hoc correlation when ``.started`` is
absent (these branches fire during admission, before the started event would
be emitted).

Consumers: ``core/handlers/frontier_dispatch.py`` (Enabled, Misconfigured).

Signals: ``pipeline.frontier.dispatch.remotemcp.{enabled,misconfigured}``.
"""
...
def PipelineFrontierDispatchRemoteMcpEnabled(  # noqa: N802
    ...
    """Emitted once per ``frontier_dispatch_v1`` execution with card-selected
    remote-connector active.
