...
| `stream` | bool | Enable streaming (pipeline ignores, always false) |
...
**Special case**: `response_format` merges from `prompt.json_schema` if not set in step (backward compatibility)
...
Prompts are structured YAML with template, system_prompt, json_schema, and generation parameters:
...
- `step.generation_parameters["response_format"]` > `prompt.json_schema` > None (backward compatibility)
- `prompt.system_prompt` > `model.system_prompt` > ""

**Note**: Generation parameters NO LONGER supported in prompts.yaml. Step config is the only source. No defaults in pipeline layer - inference engines apply their own.
