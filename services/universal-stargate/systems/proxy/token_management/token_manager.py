...
class TokenManager:
    ...
    async def _verify_model_loaded_for_token_counting(
        self,
        model_name: str,
        ...
        *,
        ...
        """
        Verify model is loaded for token counting (with waiting).

        DEPRECATED: This method waits with timeout, which is redundant after
        select_gateway_and_load_model. Use _quick_verify_model_loaded instead.

        Uses coordinator state (updated by WebSocket events) for instant verification.
        Only waits for load event if coordinator indicates model not yet loaded.

        No HTTP polling - coordinator is authoritative and always fresh.
        """
        if not self._load_waiter:
            logger.warning(
                f"⚠️ No ModelLoadWaiter available for {model_name} - "
                f"cannot verify model load. Ensure set_load_waiter() was called."
            )
            middleware_actions.append(f"model_wait_no_waiter: {model_name}")
            return False

        if not gateway_name:
            logger.error(f"❌ No gateway_name provided for {model_name}")
            middleware_actions.append("model_verification_error: no_gateway_name")
            return False

        try:
            from ..core.control_plane.model_lifecycle.waiting import LoadResult

            # Step 1: Check coordinator state
            # - sticky=True: coordinator is authoritative (single-gateway truth)
            # - sticky=False: coordinator is advisory-only (model may exist on multiple gateways)
            coordinator = self._load_waiter._global_load_coordinator
            if coordinator:
                loaded_on = coordinator.where_is_loaded(model_name)
                if loaded_on == gateway_name:
                    logger.debug(
                        f"✅ MODEL ALREADY LOADED: {model_name} on {gateway_name} "
                        f"(verified via coordinator)"
                    )
                    middleware_actions.append(
                        f"model_verified_loaded_coordinator: {model_name}"
                    )
                    return True
                elif loaded_on:
                    if sticky:
                        logger.warning(
                            f"⚠️ Model {model_name} loaded on {loaded_on} but expected on "
                            f"{gateway_name}"
                        )
                        middleware_actions.append(
                            f"model_loaded_wrong_gateway: {model_name}, "
                            f"expected={gateway_name}, actual={loaded_on}"
                        )
                        return False
                    middleware_actions.append(
                        f"model_loaded_other_gateway_non_sticky: {model_name}, "
                        f"expected={gateway_name}, actual={loaded_on}"
                    )

            # Step 1b: Coordinator doesn't confirm loaded; verify via gateway status.
            #
            # Invariant:
            #   coordinator state can be empty after Stargate restart; gateway status is
            #   the source of truth for "loaded right now".
            if gateway_instance is not None:
                try:
                    from ..core.control_plane.model_lifecycle.status import (
                        ModelStatus,
                        get_model_status,
                    )

                    status = await get_model_status(gateway_instance, model_name)
                    if not status.reachable:
                        middleware_actions.append(
                            f"gateway_unreachable_status_check: {gateway_name}"
                        )
                        return False

                    if status.status in (ModelStatus.LOADED, ModelStatus.BUSY):
                        middleware_actions.append(
                            f"model_verified_loaded_gateway_status: {model_name}"
                        )
                        return True

                    if status.status == ModelStatus.ERROR:
                        middleware_actions.append(
                            f"model_in_error_gateway_status: {model_name}"
                        )
                        return False
                except Exception as e:
                    # If status check fails unexpectedly, fall back to event-driven waiting.
                    logger.debug(f"Gateway status check failed for {model_name}: {e}")

            # Step 2: Model not loaded according to coordinator - wait for load event
            logger.info(
                f"⏳ Model {model_name} not loaded on {gateway_name}, waiting for load "
                f"(timeout={self.token_counting_timeout}s)"
            )
            middleware_actions.append(f"model_loading_wait_started: {model_name}")

            result = await self._load_waiter.wait_for_load(
                gateway_name=gateway_name,
                model_id=model_name,
                timeout=float(self.token_counting_timeout),
            )

            if result == LoadResult.LOADED:
                logger.info(
                    f"✅ MODEL LOADED (event-driven): {model_name} on {gateway_name}"
                )
                middleware_actions.append(f"model_loaded_event_driven: {model_name}")
                return True
            elif result == LoadResult.TIMEOUT:
                logger.error(
                    f"⏰ MODEL WAIT TIMEOUT: {model_name} on {gateway_name} "
                    f"after {self.token_counting_timeout}s"
                )
                middleware_actions.append(
                    f"model_wait_timeout: {model_name}, {self.token_counting_timeout}s"
                )
                return False
            elif result == LoadResult.GATEWAY_UNREACHABLE:
                logger.error(
                    f"❌ GATEWAY UNREACHABLE: cannot wait for {model_name} "
                    f"load on {gateway_name} (WebSocket disconnected)"
                )
                middleware_actions.append(
                    f"gateway_unreachable_wait: {gateway_name}, {model_name}"
                )
                return False
            else:
                logger.error(
                    f"❌ MODEL LOAD FAILED: {model_name} on {gateway_name} - "
                    f"{result.value}"
                )
                middleware_actions.append(
                    f"model_load_failed: {model_name}, {result.value}"
                )
                return False

        except Exception as e:
            logger.error(f"❌ Model verification failed: {e}")
            middleware_actions.append(f"model_verification_error: {str(e)}")
            return False
