...
def _cap_max_tokens_to_slot_context(
    ...
    Args:
        ...
        model_id: Target model (ModelId, used as dict key)
    """
    if not fed_gateway.model_resources:
        return

    model_meta = fed_gateway.model_resources.get(model_id)
    if not model_meta:
        return
...
class RequestExecutor:
    ...
    async def _execute_federated_request(self, context: RequestContext) -> Response:
        ...
        # Safety cap: ensure max_tokens does not exceed the per-slot
        # effective context.  When parallel_slots > 1, the engine splits
        # its KV cache across slots so each slot only has
        # context_length // parallel_slots usable tokens.  Token counting
        # (when it runs) already returns an effective context_limit, but
        # pipeline requests skip token counting entirely.
        _cap_max_tokens_to_slot_context(
            request_body, fed_gateway, context.selected_model
        )
