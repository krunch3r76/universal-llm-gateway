"""
Authentication client for Remote mode.

Remote initiates connection and sends auth to Master.
"""
...
class RemoteAuthClient:
    """
    Handles Remote→Master authentication.

    Remote initiates and sends credentials first.
    """
    ...
    async def authenticate(self, ws: WebSocketClientProtocol) -> bool:
        ...
        Remote sends auth first, waits for result.
        ...
        try:
            ...
            if not data.get("success"):
                ...
                logger.warning(f"Auth rejected by Master: {reason}")
                return False

            return True

        except TimeoutError:
            logger.warning("Auth timeout from Master")
            return False
