...
in-memory TTL dedupe, status presence, and source_ref resolvability on the
keyed (producer) path vs the lenient manual path.
"""
...
@pytest.mark.asyncio
async def test_manual_path_no_key_skips_resolvability(
    _stub_pipeline: list[dict[str, Any]],
) -> None:
