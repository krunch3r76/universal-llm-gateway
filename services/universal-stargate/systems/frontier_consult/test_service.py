...
async def test_team_dispatch_xai_agent_enables_mcp(
    ...
    """xAI team roles (skeptic, artisan) on grok-4.7 get mcp=True (standard card)."""
    ...
    assert caps["resolved_model"] == "xai/grok-4.7"
...
@pytest.mark.parametrize(
    ("model", "expected"),
    [
        # grok-4.7 standard card admits client-side MCP tool loop.
        ("xai/grok-4.7", True),
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # grok-4.7 honors caller intent (MCP-capable standard card).
        ("xai/grok-4.7", True, True),
        ("xai/grok-4.7", False, False),
        ("xai/grok-4.7", None, True),
    ],
)
