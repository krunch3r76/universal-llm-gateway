...
def register_agent_bus_tools(mcp: FastMCP) -> None:
    ...
    def agent_bus(tool: str, arguments: JsonArgStr = "{}") -> Any:
        ...
        Operations:
          ...
          request       (new_slug XOR thread, to='cursor', subject, body, from?, from_agent?, tags?, sidecar_content?, sidecar_slug?, desired_model?, desired_effort?, contract?, require_attended?) — life-callable Cursor Auto channel. Injects lane:cursor-auto; arms Auto when a live handler heartbeats (else handler_status=no-auto-handler); returns {thread, turn, handler_status, poll_hint}. require_attended=true (wire or DIRECTIVE body OR) ⇒ terminal status:needs-attended reason=operator_require_attended. ¬ dual-tag lane:life-to-code on degrade.
