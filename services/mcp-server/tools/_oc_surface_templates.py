...
`/mcp/code` is OFF: `team_dispatch`, `panel_dispatch`, `pipeline`, `manage`, `observability`, and every workspaces WRITE are absent from this mount — a REAL absence, not a deferred bind. Need one? Ask a code seat: `agent_bus(tool="request", to="cursor", …)` (Cursor Auto; poll `poll_hint` with `wait`) or `send` for an attended seat — skill `life-to-code-request-lane` (`lane:life-to-code`). CSE continuity hop on an existing private lane: `agent_bus(tool="hop")`, ¬ `request` + typed `TYPE: CONTINUITY_HANDOFF`. Substrate graph assert: `agent_bus(tool="substrate_graph_write")`, ¬ bus-prose for the operator to carry. Substrate friction file: `agent_bus(tool="substrate_friction_file")`, ¬ bus-prose for the operator to carry. Substrate entity mint: `agent_bus(tool="substrate_entity_mint")`, ¬ round-trip a code seat to mint an entity or owner. Gate SoT: boot card `## Dispatch & Consult` + skill `consult-routing` § Surface gate.
