...
def register_local_api_tools(mcp: FastMCP) -> None:
    ...
    def local_api(
        ...
        if svc_config is None:
            return {
                "error": (
                    f"Unknown service: {service!r}. Available: {sorted(_SERVICES)}"
                )
            }
        ...
        bearer = token or os.environ.get(svc_config["token_env"], "")
        ...
        try:
            ...
            duration = monotonic_now() - t0
            logger.error(
                "local_api relay to %s failed: %s", service, exc, exc_info=True
            )
