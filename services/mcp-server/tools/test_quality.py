...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            0,
            "check-imports: entry systems.proxy.app ENV_GAP: missing "
            "third-party module 'prometheus_client'",
            "passed",
        ),
...
def test_run_import_check_surfaces_env_gap(monkeypatch: Any, tmp_path: Path) -> None:
    repo = tmp_path / "universal-llm-gateway"
    check_script = repo / "scripts" / "check-imports"
    check_script.parent.mkdir(parents=True)
    check_script.write_text("#!/usr/bin/env python3\n")
    target = repo / "services" / "universal-stargate" / "systems" / "x.py"
    target.parent.mkdir(parents=True)
    target.write_text("pass\n")
    ...
    def fake_run(command: list[str], **kwargs: Any) -> subprocess.CompletedProcess[str]:
        return subprocess.CompletedProcess(
            command,
            0,
            stdout="check-imports: systems.x  (services/universal-stargate/systems/x.py)",
            stderr=(
                "check-imports: entry systems.proxy.app ENV_GAP: missing "
                "third-party module 'prometheus_client' — execution-environment "
                "gap, not a code regression; entry check continues"
            ),
        )
    ...
    assert result["passed"] is True
    assert result.get("env_gap") is True
