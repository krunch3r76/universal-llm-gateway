...
from tools._grokbuild_runner import _build_env
...
def _grok_models_ok() -> bool:
    ...
    A False result is NOT cached — the next call re-runs the subprocess
    check. This avoids the lru_cache cold-start poisoning pattern where a
    single early False (e.g. from the first-call transient on a fresh
    container process) would block all dispatches until MCP restarts.

    Subprocess env is ``_build_env()`` (review W3) so the preflight sees
    the same environment as the runner subprocess; otherwise the
    preflight could pass with credentials/proxy env that the stripped
    runner env lacks, masking real failures.
    """
...
def _reset_grok_models_cache_for_tests() -> None:
    """Reset the cached True state. TEST-ONLY — not a public API.

    Replaces the prior compat shim (V0) per `[universal:no-bc]`.
    Test fixtures (Phase 10) call this directly.
    """
    global _grok_models_cached
...
def validate_dispatch(  # noqa: PLR0911, PLR0913 — long admission chain by design
    ...
    *,
    ...
    if best_of_n is not None and not (
        _BEST_OF_N_MIN <= best_of_n <= _BEST_OF_N_MAX
    ):
        return _reject(
            "bad_best_of_n",
