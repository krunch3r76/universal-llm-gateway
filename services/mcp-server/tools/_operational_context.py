"""Operational context renderer — server-side protocol templates for cortex_brief.
...
def _render_observe_and_search(agent: str) -> str:
    ...
    return (
        ...
        f"{ex1}\n{ex2}\n\n"
        f"Log friction when tools or context don't work as expected, or to "
        f"park a feature ask without seeding a todo:\n"
        f"{friction}\n"
        f"Categories: feature (capability ask — no todo), tool_mismatch, "
        f"tool_absent, tool_error, schema_gap, boot_drift, lesson_gap, "
        f"lesson_conflict, stale_context, doc_drift, protocol, regression.\n\n"
...
def render_operational_context(
    ...
    sections.append(
        ...
        f"— regenerated at every cortex_brief for {agent}; do not edit -->"
    )
