"""Tests for ``tools.sidecar_schema.validate_sidecar_frontmatter``.
...
and monkeypatches ``sidecar_schema.FILES_ROOT`` to point there, so tests
...
def _schema_in_tmp(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    schema_path.write_text(
        _SCHEMA_FIXTURE.read_text(encoding="utf-8"), encoding="utf-8"
    )

    monkeypatch.setattr(sidecar_schema, "FILES_ROOT", tmp_path)
...
def test_missing_required_field_fails(field: str) -> None:
    ...
    assert any(field in err for err in result.errors), (
        f"expected error message to name missing field {field!r}; got {result.errors!r}"
    )
...
def test_schema_load_is_cached(_schema_in_tmp: Path) -> None:
    """``load_schema`` returns identical objects across calls (lru_cache)."""
    first = sidecar_schema.load_schema()
    second = sidecar_schema.load_schema()
...
def test_schema_load_survives_file_deletion(_schema_in_tmp: Path) -> None:
    ...
    process restart.
    """
    sidecar_schema.load_schema()  # prime the cache
...
def test_missing_schema_file_raises() -> None:
    ...
    sidecar_schema.load_schema.cache_clear()
    schema_path = (
        sidecar_schema.FILES_ROOT / "configs" / "schemas" / "extraction-sidecar-v1.yaml"
    )
    schema_path.unlink()
