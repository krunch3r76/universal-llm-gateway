...
def test_bare_read_sha256_round_trips_as_expected(
    sandbox_root: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    result = ops_text.write_file_impl(rel, "after", expected_sha256=bare)
    assert result["status"] == "written"
    assert (sandbox_root / rel).read_text(encoding="utf-8") == "after"


def test_mismatch_echo_canonicalizes_both_sides(
    sandbox_root: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    assert stale["reason"] == "file_sha256.mismatch"
    assert stale["expected_sha256"] == f"sha256:{'0' * 64}"
    assert stale["actual_sha256"].startswith("sha256:")
