    ...
    def cortex(tool: str, arguments: str = "{}") -> Any:
        """Cortex knowledge system — entities, assertions, relationships, edges, journals.
                ...
                Operations:
                  ...
                  session_handoff_upsert (session_id, handoff_prompt) — Upsert handoff_prompt on an already-closed session's journal row and mirror to transcript entity attributes when present. Boot omits handoffs; retrieve via entity_get(transcript:{session_id}) or journal row. 404 when session not closed; 422 when handoff_prompt empty. Idempotent upsert (replaces prior). See decision:transcript-scoped-handoff-explicit-load.
