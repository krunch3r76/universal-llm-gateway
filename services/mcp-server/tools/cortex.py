...
def register_cortex_tools(mcp: FastMCP) -> None:
    ...
    def cortex(tool: str, arguments: str = "{}") -> Any:
        ...
        Operations:
          ...
          session_close     (session_id, agent, transcript_md, summary, domains?, decisions?, open_items?, entity_ids?, prior_session_id?, dry_run?) — ATOMIC session close: validates transcript, writes file, creates entity + journal row + continues edge in one call. Rejects stubs. Pass dry_run=true for full validation (incl. audit gate) without any file write or DB row — returns {"dry_run": true, "would_succeed": true|false, ...}. Response (real call) includes transcript_entity_id (e.g. "transcript:web-YYYY-MM-DD-HHMM") — capture this and surface it to the user; use it as evidence_uri in any assertions seeded from this session.
