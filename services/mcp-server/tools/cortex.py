    ...
    def cortex(tool: str, arguments: str = "{}") -> Any:
        """Cortex knowledge system — entities, assertions, relationships, edges, journals.
                ...
                Operations:
                  ...
                  session_close     (session_id, agent, session_summary_md, summary, transcript_depth?, {transcript_jsonl_path | transcript_md}?, domains?, decisions?, open_items?, entity_ids?, prior_session_id?, handoff_prompt?, handoff_source_path?, assistant_label?, dry_run?) — ATOMIC server-side close. handoff_source_path (optional, cortex-relative) stamps handoff_provenance {write_path, written_at, source_file, source_file_sha256} on the transcript attribute (agent-bus 1188). transcript_depth ∈ {verbatim, light, none} (default verbatim). Depth selection: agent-skills/session-close-kernel.md (web/API/subagent); Cursor /session-end + session-close.mdc. verbatim: Cursor JSONL or web transcript_md required; server assembles dual-layer file + transcript entity. light: structural file only (source ignored). none: journal row + continues edge only (no transcript entity/file). transcript_jsonl_path|transcript_md required iff depth ∈ {verbatim, light}; omit both for none. 422 reasons include transcript_source.missing, transcript_jsonl.invalid, session_summary.invalid, transcript.hollow, transcript.missing_structure, summary.too_short, session_id.invalid, session.already_closed. Response always includes journal_row_id, transcript_depth; transcript_entity_id/path/content_hash null when depth=none. Quote content_hash when present (provenance-discipline.mdc). See agent_skill:session-close, agent_skill:auditor-validatable-confidence.
