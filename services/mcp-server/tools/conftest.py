...
_mcp_events = sys.modules.get("mcp_events")
if _mcp_events is not None:
    _events_file = (getattr(_mcp_events, "__file__", None) or "").replace("\\", "/")
    if "mcp-server" not in _events_file:
        del sys.modules["mcp_events"]
