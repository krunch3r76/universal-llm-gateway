...
from tools._grokbuild_validator import validate_dispatch
...
def test_continue_recent_with_session_id_yields_retired_param(
    ...
    vr = validate_dispatch("build", admission, "read_only", "sid", True, "streaming-json")
    assert vr.reason_code == "retired_param"


def test_grok_not_in_path(monkeypatch: pytest.MonkeyPatch, admission: str) -> None:
    clear_validator_caches()
    monkeypatch.setattr("tools._grokbuild_validator.shutil.which", lambda _: None)

    vr = validate_dispatch("build", admission, "read_only", None, False, "streaming-json")
...
def test_sidecar_directory_creation(
    ...
    monkeypatch.setattr("tools._grokbuild_validator._SIDECAR_DIR", sidecar)
    cwd = str(git_repo)
    install_grok_path(monkeypatch)
...
def test_cwd_missing(monkeypatch: pytest.MonkeyPatch, sidecar_root: Path) -> None:
    ...
    vr = validate_dispatch(
        "build", "/nonexistent/path/does/not/exist", "read_only", None, False, "streaming-json"
    )
...
def test_grok_models_oserror_fallback(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    from tools._grokbuild_test_support import clear_validator_caches
    from tools._grokbuild_validator import _grok_models_ok
...
def test_valid_reasoning_effort(admission: str, value: str) -> None:
    ...
    assert vr.ok is True, f"expected ok for reasoning_effort={value!r}, got {vr.reason_code!r}"
...
def test_bad_timeout_seconds(admission: str) -> None:
    vr = validate_dispatch(
        "build", admission, "read_only", None, False, "streaming-json", timeout_seconds=0
    )
    assert vr.ok is False
...
def test_bad_resume_strict_without_session_id(admission: str) -> None:
    vr = validate_dispatch(
        "build", admission, "read_only", None, False, "streaming-json", resume_strict=True
    )
    assert vr.ok is False
...
def test_retired_output_format(admission: str) -> None:
    vr = validate_dispatch(
        "build", admission, "read_only", None, False, "json"
    )
    assert vr.ok is False
...
def test_cache_reset_callable() -> None:
    ...
    from tools._grokbuild_validator import _reset_grok_models_cache_for_tests
