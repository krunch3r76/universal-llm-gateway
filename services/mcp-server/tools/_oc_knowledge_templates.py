...
probes a live Auto handler, enqueues when armed, returns `auto_handler_status` + `job_admission` + `poll_hint` — poll it with `agent_bus(tool="wait", …)`. The two answer different questions. `auto_handler_status` is the handler's heartbeat: `auto-handler-live` = a handler is up and took the job; `no-auto-handler` = the turn was written but nothing will act on it. `job_admission` is *your job's* admit-gate verdict — read `job_admission.outcome`: `refused` = it will terminal-fail, and `reason`/`fix_hint`/`missed_tokens` say why; `deferred` = the body-pure gates passed and the thread-state gates named in `coverage.deferred` have not run yet; `admitted` = the ladder completed; `not_applicable` = no job entered admit. A live handler does **not** mean your job was admitted.
...
**Substrate entity mint → `substrate_entity_mint`, ¬ round-trip a code seat to mint an entity/owner:**
`agent_bus(tool="substrate_entity_mint", arguments='{{"id": "service:slug", "type": "service", "name": "…"}}')`
— `id` + `type` + `name` required. Wraps cortex `entity_create`; 409 on collision; ¬ rich-seed; ¬ a `contract` value.
**Valid `contract` values: `answer`, `confer`, `ask`, `investigate`, `implement`, `verify`, `execute`, `propagate`, `seed`, `recon`.** `consult` is NOT a wire contract — it is deprecated and aliased to `confer`; any other unknown value is rejected 422 (`request_contract_unknown`) before the turn is written.
