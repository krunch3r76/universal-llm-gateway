...
**Valid `contract` values: `answer`, `confer`, `investigate`, `implement`, `verify`, `execute`, `propagate`, `seed`.** `consult` is NOT a wire contract — it is deprecated and aliased to `confer`; any other unknown value is rejected 422 (`request_contract_unknown`) before the turn is written.
