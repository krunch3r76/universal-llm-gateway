...
_CONDUCTOR_RUN_TO_COMPLETION_TEMPLATE = (
    ...
    "ack: wait in this dispatch for its harvest. An empty cdp review "
    "(`body_len=0`, `stall_stage=mark_terminal`, undelivered skill) is a "
    "delivery failure: retry it once, then land the lane that holds the "
    "commits. Do not `stop: PARKED_TRANSPORT` and do not discard that lane. "
    "Nest Composer "
...
_CONDUCTOR_ATTENDED_RESURFACE_TEMPLATE = (
    ...
    "is not a stop, not a page, and not a human gate. Human ROW_PINNED only "
    "when the packet or the operator explicitly names see-score or "
    "OPERATOR_GATE. Liaison IDE ≠ an operator gate."
)
