...
* **Executor** — ``bind-then-compose`` already says premium models bind and
  ...
  tier bound at ``route_policy.yaml workflows.mechanical_implement`` regardless
  of who was named, because once judgment has closed the edits are the same
  edits whoever makes them.
* **Scope** — ``NESTED_SCOPE_CONTRACTS`` already refuse a directive with no
  ...
  The waiver stays for models with ``models.<bare>.roaming: true`` and is
  withdrawn for everything else, so a premium bind must arrive with scope the
  orchestrator actually bounded.
...
def is_roaming_tier(
    ...
    *,
    ...
    return bare in reg.roaming_bare_models()


def scope_waiver_allowed(
    model_id: str | None,
    *,
    registry: WorkflowRegistry | None = None,
) -> bool:
    ...
    nobody bounded.
    """
    return is_roaming_tier(model_id, registry=registry)
...
def redirect_mechanical_executor(
    ...
    *,
    ...
    leg may be any model the orchestrator wants, the compose leg is bound at
    ``workflows.mechanical_implement``.
    ...
    reg = registry or load_workflow_registry()
    if is_roaming_tier(requested_id, registry=reg):
        return model, None
    executor_id = reg.workflows[MECHANICAL_WORKFLOW].model
    executor = resolve_desired_model(executor_id, contract=contract, registry=reg)
