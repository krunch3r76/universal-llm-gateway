...
def test_committed_checkpoint_keeps_queued_liveness_unknown() -> None:
    """A commit permits the obligation, but cannot substitute for live proof."""
    wrapper = (
        '{"schema_version":1,"status":"complete",'
        '"files_modified":["services/mcp-server/tools/cursor_request.py"],'
        '"propagation_residue":["sync_restart: mcp — manage(...)"]}'
    )
    with patch(
        "services.git_integration_worker.cursor_auto.closeout_tree_state."
        "compute_lane_a_checkpoint_value",
        return_value="committed deadbeef paths=1",
    ):
        state = compute_closeout_tree_state(
            source_repo=Path("/tmp/unused"),
            dispatch_id="d-queued-live-unknown",
    ...
    assert "live@<sha>" not in state.deployment_state
