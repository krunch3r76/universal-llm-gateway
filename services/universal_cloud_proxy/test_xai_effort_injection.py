...
async def test_effort_tier_injected(suffix: str, expected_effort: str) -> None:
    body = {"model": f"grok-4.3__effort_{suffix}", "input": "ping"}
...
async def test_bogus_suffix_passes_through() -> None:
    ...
    """
    body = {"model": "grok-4.3__effort_bogus", "input": "ping"}
    _, calls = await _run(body)
    assert calls
    upstream = calls[0]
    assert upstream["model"] == "grok-4.3", "model not stripped for bogus suffix"
