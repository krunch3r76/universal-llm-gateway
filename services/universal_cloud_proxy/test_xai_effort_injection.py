...
async def test_effort_tier_injected(suffix: str, expected_effort: str) -> None:
    body = {"model": f"grok-4.7__effort_{suffix}", "input": "ping"}
    _, calls = await _run(body)
    assert calls, "forwarder not called"
    upstream = calls[0]
    assert upstream["model"] == "grok-4.7", (
...
async def test_bogus_suffix_passes_through() -> None:
    ...
    """
    body = {"model": "grok-4.7__effort_bogus", "input": "ping"}
    _, calls = await _run(body)
    assert calls
    upstream = calls[0]
    assert upstream["model"] == "grok-4.7", "model not stripped for bogus suffix"
