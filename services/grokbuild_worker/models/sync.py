...
class WorktreeCreateRequest(BaseModel):
    ...
    name: str = Field(..., description="Short name for the worktree (no slashes or '..').")
    branch: str = Field(..., description="Branch to check out (must exist unless create_branch=True).")
    source_repo: str = Field(..., description="Absolute path to the source git repository.")
...
class PushRequest(BaseModel):
    ...
    branch: str = Field("", description="Branch to push (defaults to current branch when empty).")
    set_upstream: bool = Field(True, description="Pass -u to set the upstream tracking reference.")
...
class PRCreateRequest(BaseModel):
    ...
    pr_base: str = Field("", description="Base branch for the PR (gh default when empty).")
    pr_head: str = Field("", description="Head branch for the PR (current branch when empty).")
    draft: bool = Field(False, description="Create as a draft PR.")


ResultFormat = Literal["json", "text", "summary"]
