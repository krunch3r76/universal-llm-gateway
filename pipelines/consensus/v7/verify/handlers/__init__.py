...
def register_handlers(router) -> None:
    ...
    router.register_domain_handler_class(
        "consensus", "consensus_decompose_v7", DecomposeHandler
    )
    router.register_domain_handler_class(
        "consensus", "consensus_classify_domain_v7", ClassifyDomainHandler
    )
    router.register_domain_handler_class(
        "consensus", "consensus_atomicity_gate_v7", AtomicityGateHandler
    )
    router.register_domain_handler_class(
        "consensus", "consensus_contextualize_v7", ContextualizeHandler
    )
    router.register_domain_handler_class(
        "consensus", "consensus_domain_verify_v7", DomainVerifyHandler
    )
    router.register_domain_handler_class(
        "consensus", "consensus_verify_general_v7", VerifyGeneralHandler
    )
    router.register_domain_handler_class(
        "consensus", "consensus_filter_threshold_v7", FilterThresholdHandler
    )
