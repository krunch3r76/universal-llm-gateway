"""
Iterative check→revise loop to ensure all verified facts appear in the answer.

After combine produces a base answer from verified facts, this handler checks
which facts are missing and revises the answer to weave them in. Up to
max_review_rounds iterations; exits early when no facts are missing.
...
def _parse_missing_indices(raw_response: str, fact_count: int) -> list[int]:
    ...
    try:
        ...
        logger.warning("Failed to parse check_synthesis response: %s", e)
        return []
...
class EnrichReviewerHandler(BaseHandler):
    """Check answer for missing facts and iteratively revise to include them."""

    step_type: str = "consensus_enrich_review_v6_1"
    ...
    async def execute(
        ...
        """Review completeness; iterative check→revise up to max_rounds."""
    ...
    async def _check_and_revise(
        ...
        """One round: check for missing facts; revise if any. Returns (answer, missing_count, pt, ct)."""
        check_prompt_ref = step.prompt_ref
    ...
    def validate(self, step: StepConfig) -> list[str]:
        ...
        if not step.prompt_ref:
            errors.append(f"Step '{step.id}' missing prompt_ref (check_synthesis)")
