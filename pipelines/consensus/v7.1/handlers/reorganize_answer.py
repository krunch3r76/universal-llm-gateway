...
class ReorganizeAnswerHandler(BaseHandler):
    ...
    step_type: str = "consensus_reorganize_v7_1"
    ...
    def validate(self, step: StepConfig) -> list[str]:
        ...
        for field in ("answer", "question", "incorporated", "excluded_with_reason"):
            if field not in inputs:
                errors.append(
                    f"Step '{step.id}': consensus_reorganize_v7_1 requires '{field}' "
                    "in handler_inputs"
                )
        if not step.prompt_ref:
            errors.append(
                f"Step '{step.id}': consensus_reorganize_v7_1 requires 'prompt_ref'"
            )
        return errors
