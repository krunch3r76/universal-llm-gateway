...
class IdentifyRedundancyHandler(BaseHandler):
    ...
    step_type: str = "consensus_identify_redundancy_v7_1"
    ...
    def validate(self, step: StepConfig) -> list[str]:
        ...
        for field in ("answer", "question"):
            if field not in inputs:
                errors.append(
                    f"Step '{step.id}': consensus_identify_redundancy_v7_1 requires '{field}' "
                    "in handler_inputs"
                )
        if not step.prompt_ref:
            errors.append(
                f"Step '{step.id}': consensus_identify_redundancy_v7_1 requires 'prompt_ref'"
            )
        return errors
