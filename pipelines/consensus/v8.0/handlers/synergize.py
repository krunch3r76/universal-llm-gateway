...
from ._lib._dedup_clustering import cluster_similar
from ._lib._dedup_embeddings import get_embeddings
...
class SynergizeHandler(BaseHandler):
    ...
    step_type: str = "consensus_synergize_v8_0"
    ...
    async def execute(
        ...
        if threshold >= 1.0:
            # At threshold 1.0, no pair can merge; skip embedding RPC entirely.
            clusters = [[i] for i in range(len(all_facts))]
            cluster_stats = {
                "pairs_compared": 0,
