...
from ..._lib._chain_verification import verify_claims
...
class VerifyGeneralHandler(BaseHandler):
    ...
    step_type: str = "consensus_verify_general_v8_0"
    ...
    def _resolve_verify_models(
        ...
        Returns (active_aliases, full_pool_aliases) where full_pool_aliases
        is the pool before self-exclusion — used by the viewer to show the
        originator column even though it didn't vote.
        """
    ...
    async def execute(
        ...
        # Build execution configs per model, capping at step chunk_size
        registry = context._registry
        exec_configs: dict[str, ModelExecutionConfig] = {}
        step_chunk = (
            context.pipeline.options.get("verify_chunk_size")
        ...
        if step_chunk is None:
            raise ValueError(
                ...
                "or verify_chunk_size, or step domain field chunk_size"
            )
        ...
        for alias in verify_model_aliases:
            ...
            exec_config.chunk_size = min(step_chunk, exec_config.chunk_size)
        ...
        return StepOutput(
            ...
            json={
                ...
                "verifier_pool": full_verifier_pool,
