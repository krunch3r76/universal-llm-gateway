...
The architecture is a direct generalization of the existing consensus-pipeline invariant in `universal-llm-gateway/libs/provenance`: where the pipeline today gates inter-model verification on `originator_model_id != evaluator_model_id`, this spec generalizes the originator slot from "another model" to "a primary-source authority entity," so the same `is_independent` check and the same lineage tracking carry over. Gap detection generalizes the *role* of `validate_provenance_present` (presence of provenance fields), not that function's call site — see §7 and the WWP r3 substrate-debt detector.

**Claim-record frame (2026-07-16).** A factual artifact is a *rendering* of a claim record: each material claim carries an epistemic state and a rendering disposition, bound by a compatibility law (no rendering stronger than the epistemic state supports). Obligations grade by blast radius and land at three durability tiers — in-session ledger, greppable DISPOSITIONS / gap grammar, entity-backed graph. This spec is the **tier-3 / publish-gate** instantiation for authored artifacts; compose-time law lives in `agent_skill:writing-with-provenance` (vision: `cortex://notes/system/threads/5195-fable-writing-provenance/fable-vision-dialectic.md`).

The load-bearing public claim: **structural inaccessibility of *unacknowledged* un-backed claims at the counterparty/filed tier** — once detectors and publish gates are live, the graph refuses to treat a claim as resolved if its citation does not point to a typed authority entity (or a *named* URI-fallback substrate-debt deferral), and refuses to mark a quotation `confirmed` if an independent model has not read the source and corroborated. URI/file-locator fallbacks on substrate-derived artifacts are **deferrals that mint graph debt**, not silent exemptions. Absolute "impossibility by construction" is the destination of the gated pipeline, not a description of today's compose-time seat alone.

---

## § 0. Strategic frame

The architectural thesis (assertion 9149) is that the consensus pipeline's core invariant — **independence + provenance + automated verification gates produce higher-quality outputs** — generalizes from "originator = another model" to "originator = a primary-source authority." The pipeline architecture, the `is_independent` check, and the gap-detection *role* (presence of provenance) all carry over; the concrete detector surfaces evolve (citation-token structural gaps here; named URI-fallback substrate debt under WWP r3 — see §7).
...
**Blast-radius grading (pointer).** Wider external consequence ⇒ lower materiality floor, more total claim-record decomposition, more durable tier required. Counterparty / filed / published is the top tier (total ledger membership; floor gates verification depth). Substrate-derived / durable-internal uses the ratified materiality floor plus named URI-fallback debt. Conversational drafts stay off the graph unless invoked. Full graded law: `agent_skill:writing-with-provenance`; this spec owns the entity-backed / publish-gate end of that ladder.
...
**Ancestry, not call-site.** §7 generalizes the *role* of `libs/provenance/cross_model.py::validate_provenance_present` ("is provenance present?") from consensus pipeline items to authored artifacts. That function itself validates consensus provenance *dicts* (`originator_model_id`, etc.) — it is **not** the insertion point for artifact DISPOSITIONS / URI-fallback debt. The WWP r3 arm is a separate FS-touching cortex audit detector (`substrate_debt_uri_fallback`, sibling of `provenance_cites_staging`); densify: `cortex://notes/system/threads/5195-fable-writing-provenance/gap-detector-densify.md` · `todo:wwp-substrate-debt-gap-detector`. This section remains the citation-token → entity structural program; r3 is the named-deferral debt arm — complementary, not a rewrite of 7.1.
...
All five finding kinds (`contradicted_claim`, `verbatim_check_failed`, `missing_backing_entity`, `unverified_entity`, `unverified_claim`) surface through the same channel; severity determines render-gate enforcement (§ 6.2) and review-queue prioritization. Named URI-fallback substrate debt (r3) surfaces via the separate audit-detector kind above — drafting seats *name* deferrals in DISPOSITIONS; the detector mints debt; that path is complementary to the five citation-token kinds here.

### 7.5 The structural-impossibility guarantee

Once `detect_structural_gaps(artifact, case)` returns zero `critical` and zero `high` findings AND `verification_coverage(synthesis_entity)` shows zero unverified or contradicted claims, the artifact cannot contain *unacknowledged* hallucinated citations by construction at the publish gate. Every citation token resolves to an entity **or** is carried as named URI-fallback substrate debt (r3) awaiting gardening; every entity has a `source_uri` to a primary source; every quotation has been verbatim-checked or flagged; every claim has been corroborated by an independent verifier or contradicted (and contradictions surface as critical findings before render).

This is the load-bearing market story per assertion 9149, graded by blast radius (Abstract). The *Mata v. Avianca* failure mode is the citation token that resolves to nothing in the graph (`missing_backing_entity`) **without** a named deferral. The *Park v. Kim* failure mode is the citation that resolves to an entity that was never fact-checked against its `source_uri` (`unverified_entity` or `unverified_claim`). Both are detected mechanically, not by reviewer attention. A third failure mode — the claim that an independent verifier read and *rejected* — becomes a `contradicted_claim` finding at `critical` severity, gating render until resolved. A fourth — URI/file fallback used as a silent exemption — is closed by r3: fallbacks must be named so debt is mintable.
