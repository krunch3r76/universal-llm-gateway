...
`corpus_hints_path` (optional): path to `corpus_hints.yaml` (scope → vocabulary hints for the `suggest_terms` / `filter_corpus_hints` retrieval steps). Generated by `python -m services.rag.corpus_hints` from the property index.
...
# Optional: path to corpus hints for scope-specific vocabulary enrichment
corpus_hints_path: ~/.rag/corpus_hints.yaml
