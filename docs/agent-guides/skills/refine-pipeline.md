...
result = pipeline(pipeline="{pipeline-id}", messages=[{"role": "user", "content": "test input"}])
...
### MCP path (preferred — single call)

Use `pipeline_consult` for step-specific advice. It queries step metadata from
the event service, auto-detects the RAG scope from the model tier, and runs the
`consult-prompt-engineer` pipeline with grounded research:

```
pipeline_consult(
    execution_id=EXEC_ID,
    step_name="{step_name}",
    problem="The output has redundant paragraphs and ignores the length constraint. "
            "Prompt: {paste relevant prompt text}. "
            "Output: {paste problematic output excerpt}."
)
→ {"advice": "...", "scope_used": "research_small_llm", "model": "...", "duration_s": 12.3}
```

Include as much context as possible in `problem` — the prompt text, model output
excerpt, and what specifically is wrong. The tool fetches step metadata (model,
timing, tokens) automatically from the execution trace.

**Scope auto-detection** (override with `scope` parameter when needed):
...
result_b = pipeline(pipeline="{pipeline-id}", messages=[{"role": "user", "content": "same test input"}])
...
validate_pipeline(pipeline="{pipeline-id}")

# Full end-to-end run
final = pipeline(pipeline="{pipeline-id}", messages=[{"role": "user", "content": "test input"}])
...
- `cortex:agent-skills/implementation-plan-workflow.md` — when a refinement cycle is one phase of a multi-phase plan deck (e.g. `tmp/prompts/{name}/phase-N-refine-{pipeline-id}.md`). The phase-doc contract still applies: BEFORE/AFTER blocks for sandbox-staged prompt edits, verification via `compare-runs`.
- `cortex:agent-skills/ulg-architecture.md` §Event Service Primary — for diagnosing pipeline behavior via the event service when this skill's `signal-events` queries surface anomalies that point at infrastructure rather than prompt issues.
