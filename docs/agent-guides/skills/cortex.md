---
name: cortex
description: Tier-2 depth home for the cortex MCP tool — full op catalog, taxonomies, workflow chains, and behavioral contracts. Load when authoring, deciding, or debugging Cortex calls beyond the in-band tool contract.
trigger_match_terms: ["cortex", "entity_get", "assert", "supersede", "session_close", "dispatch", "cortex-op", "derivation_type", "relationship_create", "edge_create"]
related_skills: ["cortex-orientation", "cortex-provenance-discipline", "cortex-entity-restructure", "cortex-v24-implementation-arc", "subgraph-render", "session-close", "auditor-validatable-confidence", "consensus-steelman-posture"]
---
...
**Access path invariant:** all Cortex reads/writes route through `cortex(...)` / `cortex_boot(...)` / `local_api(service="cortex-api", ...)`. Never use direct SQL against the cortex DB.
...
**Entity ID convention:** `{type}:{slug}` — e.g. `person:kaywan-mansubi`, `decision:eml-mcp-support`, `todo:slug`. List `entities(type=...)` before creating to avoid duplicates.
...
| `session_close` | **Atomic** server-side close. Required: `session_id`, `agent`, `session_summary_md`, `summary`. `transcript_depth` ∈ {`verbatim`,`light`,`none`} (default `verbatim`). **`transcript_jsonl_path` or `transcript_md` required iff depth=`verbatim`**. `handoff_prompt`/`handoff_source_path` require depth ∈ {`light`,`verbatim`}; depth=`none` incompatible with handoff. Handoff derivation from HTML comment markers in source file; TOCTOU guards via `expected_*` sha fields → 409 on mismatch. `dry_run` previews without writes. `source_ref` canonicalized server-side; unparseable closes with provenance note (never 422). Response: `journal_row_id`, `transcript_depth`; transcript fields null when depth=`none`. `422 session.already_closed` carries prior close IDs — not failure. Returns 201 with `transcript_entity_id`, `transcript_path`, `content_hash`, `turn_count`, `byte_count` on success. See `agent_skill:session-close`. |
| `session_close_preflight` | Validate args + path sandbox + assemble in memory + audit-gate without writing. Returns `{ok, turn_count, byte_count, audit, warnings}` or `{ok:false, error, reason}`. Run before committing close. |
...
| Op | Purpose · contract · params |
|---|---|
| `observe` | Lightweight observation. Defaults: `entity_id=person:kaywan`, `confidence=believed`, `derivation_type=agent_observation`. |
| `friction` | Log tool/schema/boot friction — **observation only, NOT ticket channel**. `owner`: bare slug → `service:{slug}` or full `service:`/`agent_skill:`/`ai_agent:` id (non-service owners must exist). `service=` back-compat alias. Categories: `tool_error`, `tool_mismatch`, `tool_absent`, `schema_gap`, `boot_drift`, `lesson_gap`, `lesson_conflict`, `stale_context`, `doc_drift`, `protocol`. Actionable rows → codified bug cycle (investigate → execute); see `agent_skill:friction-review`. |
...
| `friction_close` | Close friction after fix. Creates closure assertion (`derivation_type=commitment`, `confidence=confirmed`) + `resolves` edge via supersede. **`resolution_kind`:** `agent_skill:{slug}`, `workflow:{slug}`, `todo:{slug}`, `superseded`, `wontfix`. May auto-promote to recon-pending todo when `resolution_kind=todo:{slug}`. *(In `cortex-deep-ref.mdc`; absent from tool docstring.)* |
...
2. **`cortex_boot` parameter shape:** `cortex-essentials.mdc` / `cortex-deep-ref.mdc` use `cortex_boot(agent="cursor")`. Injected `cortex-orientation` uses `cortex_boot(family="claude", platform="cursor")`. `tool-reference.md` §cortex_boot documents `family`+`platform` with default `claude-cursor`. **Boot is a separate MCP tool** — not in this skill's op catalog; conflict noted for orientation cross-ref.
...
10. **`observe` default entity:** `cortex-deep-ref.mdc` examples omit default; tool docstring implies `person:kaywan` default — confirm at call time.
