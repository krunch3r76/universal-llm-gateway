...
Event/API/runtime contract changes ⇒ `docs/event-contracts.md` audit. The catalog **table regions** are GENERATED by `scripts/gen-event-catalog` (deterministic AST walk of `@event_factory` call sites across `services/`, `libs/`, `systems/`); the **contract prose** (Event Lifecycle Contracts, Signal Classification, forensics narratives, invariants, query examples) is manually curated OUTSIDE `<!-- GENERATED:START -->` / `<!-- GENERATED:END -->` markers. ¬hand-edit inside a GENERATED region — edit the factory and regenerate. `gen-event-catalog --check` enforces code↔doc parity across ALL domains.  
Applied handoff findings that change event semantics, failure modes, coordination behavior, API surfaces, or runtime/user-visible contracts MUST trigger a documentation audit before closure: factory/payload-key changes ⇒ regenerate; semantics/prose changes ⇒ curate outside markers. Other manual docs follow the same audit invariant.
...
∀ row R in a GENERATED region ⊆ docs/event-contracts.md : R is derived from a live factory ⇒ ¬∃ live_factory(R.signal) ⟹ R vanishes on regen (¬hand-delete)  
∀ factory F added/changed ⇒ run `scripts/gen-event-catalog` (regenerates R)  
∀ payload_field_change(F) ⇒ regenerate (static dict-literal keys) ∨ update the dynamic-signal annotation/exceptions entry

Rows are tied to live factories by signal and maintained by **regeneration**, not hand-edit. No row may outlive its factory (the generator drops it); no factory may ship without its row (`gen-event-catalog --check` blocks). Curated prose changes remain same-change manual edits outside markers. (Intentional overlap with event-factory dormancy in `[quality]` — now mechanically enforced by `--check`.)
