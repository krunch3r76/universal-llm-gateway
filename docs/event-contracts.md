...
| `rag.file.indexing.gated` | `file`, `layer` | file in an entity-gated watch root skipped because no cortex entity backs it via `source_uri`. `layer` ∈ {`watcher_sweep`, `index_funnel`}. role=coordination. **Not** a failure row. |
