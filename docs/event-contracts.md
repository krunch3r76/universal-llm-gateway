...
| `frontier.endpoint.requested` | `request_id`, `has_tools` | `agent` (nullable), `model` (nullable) | endpoint admission request observed before persona enforcement and dispatch forwarding. Post-D, distinguish team-vs-raw by `agent IS NOT NULL`; the historical caller-facing `boot` field is removed. (node-scoped) |
...
| `pipeline.frontier.dispatch.completed` | `agent` (nullable), `execution_id`, `turns_used`, `tool_calls_made`, `reasoning_present`, `prompt_tokens`, `completion_tokens`, `provider`, `op` | native-endpoint loop returned terminal content; `op` ∈ {`generate`, `to_thread`, ``} (node-scoped) |
| `pipeline.frontier.dispatch.empty.completion` | `execution_id`, `agent` (nullable), `model`, `provider`, `turns_used`, `tool_calls_made`, `finish_reason`, `block_reason` | Fires when `frontier_dispatch_v1` returns empty/whitespace-only content on the non-exhausted branch. Distinct from `.exhausted` (intentional no-content on max-turns). Emitted immediately before `EmptyCompletionError` is raised so terminal state converts from `completed` to `failed`. (node-scoped) |
| `pipeline.frontier.dispatch.exhausted` | `agent` (nullable), `execution_id`, `turns_used`, `tool_calls_made`, `provider`, `op` | native-endpoint loop hit `max_tool_turns` without terminal content; `op` ∈ {`generate`, `to_thread`, ``} (node-scoped) |
...
| `pipeline.frontier.dispatch.boot.mismatch` | `execution_id`, `agent`, `provider`, `boot_mode`, `reason` | Emitted when `frontier_dispatch_v1` rejects a handler-derived dispatch tier/provider pair that would advertise a client-side MCP tool surface the provider cannot execute — the "hydration paradox". Structural case (legacy): `provider='xai'` + `boot_mode='team'`. Now uses silent coercion for standard xAI multi-agent cases. (node-scoped) |
...
| `pipeline.frontier.dispatch.started` | `execution_id`, `agent` (nullable), `model`, `provider`, `boot_level`, `remote_mcp`, `op` | Fires once per `frontier_dispatch_v1` execution, after hydration (if persona) and before the native call. `boot_level` is internal observability vocabulary derived from agent presence, not a caller-facing parameter. `op` ∈ {`generate`, `to_thread`, ``}. Pipeline-surface analog of the legacy `mcp.frontier.generate.called` signal; `_provider_health` aggregates both into its `called` bucket during the Phase-1→Phase-2 co-emit window (node-scoped) |
...
| `mcp.frontier.generate.called` | `model`, `reasoning_effort` (MCP-server `record()` fields; infrastructure adds `request_profile`, `client_ip`, `auth_mode`, `mcp_method`, `tool_name`, `jsonrpc_id`) | Emitted by `services/mcp-server/tools/frontier.py` `record()` at the start of each `frontier_generate` MCP call (persona-free door). `reasoning_effort` is `""` when not passed. `agent` and `boot` were removed at the Phase 3-D surface split — persona dispatches emit `mcp.team.generate.called` instead, and `boot` was eliminated entirely (no caller-facing toggle remains). Co-emits with `pipeline.frontier.dispatch.started` (pipeline surface) |
| `mcp.team.generate.called` | `agent`, `model`, `reasoning_effort` (MCP-server `record()` fields; infrastructure adds `request_profile`, `client_ip`, `auth_mode`, `mcp_method`, `tool_name`, `jsonrpc_id`) | Emitted by `services/mcp-server/tools/frontier.py` `record()` at the start of each `team_generate` MCP call (persona-required door). `reasoning_effort` is `""` when not passed. Sister signal of `mcp.frontier.generate.called` — split at Phase 3-D so persona vs raw dispatches can be aggregated separately |
| `mcp.team.generate.dispatched` | `execution_id` | Emitted from `_relay` when Stargate `/api/v1/team/generate` returns 2xx (the dispatch envelope). `execution_id` is the pipeline run ID for caller polling; may be `""` if upstream omitted it. Indicates admission succeeded; terminal outcome is observable on the pipeline surface (`pipeline.frontier.dispatch.{completed,exhausted,failed}`) |
| `mcp.team.generate.rejected` | `status`, `field` | Emitted from `_relay` when Stargate returns a 4xx (caller error: persona violation, `extra="forbid"` validation failure, etc.). `status` is the HTTP status; `field` is the offending field name when available (`""` otherwise; populated from `error.field` for `FrontierEndpointError`-shape envelopes and from `detail[0].loc[-1]` for FastAPI 422 list-detail). Distinct from `.failed` which is server/transport failure |
| `mcp.team.generate.failed` | `error` (`"transport"` \| `"non_json"` \| `"upstream"`), `status` (omitted for `"transport"`) | Emitted from `_relay` for non-caller failures: `"transport"` = `httpx.RequestError` (Stargate unreachable); `"non_json"` = response body could not be parsed as JSON; `"upstream"` = HTTP 5xx from Stargate (e.g. `pipeline_dispatch_unavailable` 503). Distinct from `.rejected` which is caller error |
| `mcp.frontier.generate.dispatched` | `execution_id` | Sister of `mcp.team.generate.dispatched` for the persona-free door (`/api/v1/frontier/generate`). Same semantics |
| `mcp.frontier.generate.rejected` | `status`, `field` | Sister of `mcp.team.generate.rejected` for the persona-free door. Same semantics |
| `mcp.frontier.generate.failed` | `error` (`"transport"` \| `"non_json"` \| `"upstream"`), `status` (omitted for `"transport"`) | Sister of `mcp.team.generate.failed` for the persona-free door. Same semantics |
