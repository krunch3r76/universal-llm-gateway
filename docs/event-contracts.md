...
| `pipeline.frontier.dispatch.boot.mismatch` | `execution_id`, `agent`, `provider`, `boot_mode`, `reason` | Emitted when `frontier_dispatch_v1` rejects a handler-derived dispatch tier/provider pair that would advertise a client-side MCP tool surface the provider cannot execute — the "hydration paradox". Structural case: `provider='xai'` + `boot_mode='team'` (agent set, `mcp_enabled=True`) — xAI multi-agent models reject client-side function tools, but the assembled system prompt includes `CORTEX_TOOL_QUICKREF`. Fix: pass `tools=[]` explicitly via `team_generate`, or `mcp=False`. `boot_mode` is internal observability vocabulary, not a caller-facing parameter. Precedes `pipeline_execution_failed` with `code=boot_provider_mismatch`. (node-scoped) |
