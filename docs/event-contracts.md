...
| `pipeline.frontier.dispatch.tool.suppressed` | `execution_id`, `agent`, `model`, `provider`, `reason` | Emitted when dispatch handler silently coerces tool surface for provider limitation (e.g. xAI multi-agent `reason="xai_multi_agent_client_tools_unsupported"`). Implements "tools not a caller concern" — no error to caller, telemetry only. Pairs with `CORTEX_TOOL_QUICKREF` suppression and `mcp_tool_loop=False`. (node-scoped) |
...
| `pipeline.frontier.dispatch.started` | `execution_id`, `agent` (nullable), `model`, `provider`, `boot_level`, `remote_mcp` | Fires once per `frontier_dispatch_v1` execution, after hydration (if persona) and before the native call. `boot_level` is internal observability vocabulary derived from agent presence, not a caller-facing parameter. Pipeline-surface analog of the legacy `mcp.frontier.generate.called` signal; `_provider_health` aggregates both into its `called` bucket during the Phase-1→Phase-2 co-emit window (node-scoped) |
