...
| X6 | Session close ledger | coordination | `session_journals`: one row per close; lid facet (episodic) + window facet (sealed speech registry); views `session_lids` / `session_windows`; RAG indexes the derived transcripts directory, not rows | `GET /session-journals`, `POST /session-journals` | `journal_read`, `journal_write` |
