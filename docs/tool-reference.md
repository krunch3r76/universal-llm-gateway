...
| `role` | API (`generate`/`to_thread`): `reviewer`, `gatherer`, `synthesizer`, `artisan`, `skeptic`. Handoff only: `web-consult`, `web-implement`, `cursor-consult`, `cursor-implement` | Functional role roster only — **not** a substrate selector. SDK peer slug via `role=` → 422 `role_is_not_a_seat` (use `seat=`). Seat aliases like `claude-web` → 422 `handoff_role_invalid`. **`skeptic`**: default `xai/grok-4.7` (MCP-capable standard card) — pre-stage context on `dispatch_thread_id`; admission returns `capabilities.inline_only` / `capabilities.mcp_enabled`. |
| `seat` | Auto-dispatch (`generate`/`to_thread`): `cursor-sdk`. Handoff: manual bus seats (`web-anthropic`, `cursor`, …) — not auto seats | Mutually exclusive with `role` on generate/to_thread. SDK implement peer: `seat=cursor-sdk`. `op=handoff` + auto seat → 422 `seat_not_manual`. **`seat=cursor` on `op=generate`** is invalid (handoff seat — use `seat=cursor-sdk` for Cursor-substrate workers). |
| `lane` | `"A"\|"B"` | **Required** on top-level `seat=cursor-sdk` generate/to_thread unless `nest_under`/`resume_of` inherits (422 `lane_required` if omitted). In-repo implement → `B`. Bind-only / bus-only / CHECKPOINT tip author → **`A`** + reason in brief. See skill `consult-routing` § cursor-sdk checkout lane. |
...
(`cursor-consult`, `cursor-implement`). web-anthropic doing local file work should
...
3. `agent_bus(tool="wait", arguments=poll_hint.arguments_json)` — or build the same
   ...
   `completion=first_reply_from` or `proof_reply_from` for CDP handoffs, `from_agent`).
   Re-call with `wait_seconds` 0 (snapshot) or up to 300 (server-side block) until
   `complete=true` and `status=complete`. Prefer `poll_hint.arguments_json` —
   Cursor-IDE seats receive `wait_seconds=0`; web/API keep 60 (life MCP
   client ceiling; friction 24081 + Fable observe 2026-08-03).
   Attended spinner >5 min on wait → interrupt and re-poll with `wait_seconds=0`.
...
skeptic + reviewer MCP-capable (grok-4.7 standard card)). Each member `dispatches` entry may also
...
| Op | Args | Description |
|---|---|---|
| `threads` | status?, to?, last? | List threads. status: active/archived/all |
...
| `request` | new_slug XOR thread, to=`cursor`, subject, body, contract?, desired_model?, desired_effort? (omit/`auto` ⇒ per-contract default: judgment contracts `xhigh`, mechanical `medium`; explicit rung always honored), require_attended?, … | **Cursor Auto channel** — arms handler when live; returns `{thread, turn, auto_handler_status, job_admission, poll_hint}`. `auto_handler_status` ∈ `auto-handler-live\|no-auto-handler\|static-pin-refused\|enqueue_failed` is the **handler's** heartbeat and never a job verdict. `job_admission` is **this job's** admit-gate verdict: `outcome` ∈ `refused\|deferred\|waived\|admitted\|not_applicable`, `coverage.asserted`/`coverage.deferred` name which of the ladder's gates crossed the wire (body-pure gates assert synchronously; thread-state gates report `deferred`, never admitted), and a refusal additively carries `fix_hint` + `missed_tokens`. `agent_bus_read(job_state)` remains authority-of-record and the recovery path. `contract` ∈ `answer\|confer\|investigate\|implement\|verify\|execute\|propagate\|seed\|recon`. `recon` = cheap recon findings before implement or escalate. `execute` = one tier-M allowlisted op (`tool_op:` + `effects_expected:`; `manage.*` denied). `propagate` = operator restart request (propagation ledger + drain-gated `sync_restart`; body: `scope: propagation sync_restart <service>` or `## propagation` YAML + `effects_expected:`). `seed` = closable work item (architecture may be open). Poll via `wait` from `poll_hint`. |
| `hop` | thread, reason, from?, cse_chat_url?, cse_registration_id?, desired_model?, desired_effort? (omit/`auto` ⇒ per-contract default: judgment contracts `xhigh`, mechanical `medium`; explicit rung always honored), … | **Continuity hop verb** — authors `TYPE: CONTINUITY_HANDOFF` and enqueues with `continuity_hop=true`. Returns `{thread, turn, auto_handler_status, job_admission, continuity_hop, poll_hint, successor}`. Reports handler liveness, never `status:done`; a hop routes around admit, so `job_admission.outcome` is `not_applicable` (`reason: continuity_hop_route`). Not a contract token. |
| `lane_bind` | thread, parent_thread, lane_role, bound_by?, evidence? | **Lane parentage verb** — appends a child-lane association. `lane_role` ∈ `sub_mission\|hop\|spillover\|dispatch\|side\|parallel`; distinct from `branch_associate` (git). Not a contract token. |
| `lane_current` | thread | **Lane parentage read** — returns the derived current association (`state=none` when unbound). Not a contract token. |
