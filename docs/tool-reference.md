...
| `dispatch_thread_id` | `str` | Compaction key for server-owned thread persistence (`thread:dispatch:{id}`). Stable per arc/session. Context for non-packet dispatches is read from this thread's latest turn body. For `seat=cursor-sdk` generate with `packet_path`, the packet is the instruction channel (bus turn ignored when both are present). Unused by `op="handoff"`. |
