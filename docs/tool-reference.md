...
| `request` | new_slug XOR thread, to=`cursor`, subject, body, contract?, desired_model?, desired_effort?, require_attended?, … | **Cursor Auto channel** — arms handler when live; returns `{thread, turn, handler_status, poll_hint}`. `contract` ∈ `answer\|confer\|investigate\|implement\|verify\|execute\|propagate`. `execute` = one tier-M allowlisted op (`tool_op:` + `effects_expected:`; `manage.*` denied). `propagate` = operator restart request (propagation ledger + drain-gated `sync_restart`; body: `scope: propagation sync_restart <service>` or `## propagation` YAML + `effects_expected:`). Poll via `wait` from `poll_hint`. |
